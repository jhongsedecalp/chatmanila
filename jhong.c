 ...
 *****************************************************************************
      (6/2002) All changes to the code made by Jhong and Kelotz     
*/
#include <stdio.h>
#ifdef _AIX
...
//Header files kelz
#include "headerfiles/atmos.h"
#include "headerfiles/jhong.h"
#include "headerfiles/prototypes.h"
#include "headerfiles/commands.h"
#include "headerfiles/money.h"

//Source files kelz
#include "srcfiles/atmos.c"
#include "srcfiles/sreboot.c"
#include "srcfiles/socials.c"
#include "srcfiles/kill.c"
#include "srcfiles/wizard.c"
#include "srcfiles/money.c"


// The following ifdef must be made after all of the above.
#ifdef NETLINKS
  #include "headerfiles/netlinks.h"
  #include "srcfiles/netlinks.c"
#endif
#include "srcfiles/matrixbot.c"
...
/* finish off the boot-up process */
    if (argc > 2 && atoi(argv[2]) > 1 ) //added for .sreboot
	do_srload();
...
GOT_LINE:
	    ...
	    if (!user->misc_op) {
                if (strlen(inpstr) <= strlen(user->inpstr_old))
                    len = strlen(inpstr);
                else
                    len = strlen(user->inpstr_old);

                if ((!strncmp(inpstr, user->inpstr_old, len) || !strcmp(inpstr, ".")) && user->room != get_room(default_shoot))
                    user->spams++;
                else
                    user->spams = 1;

		if (user->spams >= MAX_SPAMS) {
                    write_user(user, "~OLYou are being kicked for spamming. So stop it.\n");
	 	...
	 	if (!strcmp(inpstr,".") && user->inpstr_old[0]) {
		    ...
		    vwrite_user(user,"%s\n",inpstr);
		} else { //else save current one for next time
		...
		if (user->afk==2) {
		    ...
		    cls(user);
		    write_user(user,"~OL~FYSession unlocked, you are no longer ~FRAFK.\n");
		} else write_user(user,"~OL~FWYou are no longer ~FRAFK.\n");  
		user->afk_mesg[0]='\0';
		if (user->afkbuff[0][0]) 
		    write_user(user,"\n~OLYou have some tells in your afk review buffer.  Use ~FYrevafk~FW to view them.\n\n");
		if (user->vis)
		    vwrite_room_except(user->room,user,"%s~RS ~OLcomes back from being ~FYAFK.\n",user->recap);
		if (user->afk==2) {
...
char *long_date(int which) {
    ...
    if (which)
	sprintf(dstr,"on %s %d%s %s %d at %02d:%02d%s",day[twday],tmday,ordinal_text(tmday),month[tmonth],(int)tyear,hour,(int)tmin,!ap?"am":"pm");
    else
	sprintf(dstr,"[ %s %d%s %s %d at %02d:%02d%s ]",day[twday],tmday,ordinal_text(tmday),month[tmonth],(int)tyear,hour,(int)tmin,!ap?"am":"pm");
...
void create_system(void) {
    ...
    amsys->rs_which=-1;
    amsys->rs_user=NULL;
    amsys->gatecrash_level=BOT+1; /* minimum user level which can enter private rooms */
    amsys->min_private_users=2; /* minimum num. of users in room before can set to priv */
    amsys->ignore_mp_level=BOT; /* User level which can ignore the above var. */
...
// If you must add to this list, please add to the bottom
...
if (found >1) {
  write_user(user, "~OLName is not unique.\n\n");
...
RM_OBJECT get_room(char *name) {
	...
	if (!strncmp(rm->lowername, temp, strlen(temp)))
	    return rm;
    }
...
user->room=rm;
if (user->lroom==2) vwrite_user(user,"\nYou have been ~FRshackled~RS to %s.\n\n",rm->name);
else vwrite_user(user,"\nYou are connecting into the %s room.\n\n",rm->name);
}
...
if (!num) user->port=port[0]; 
  ...
  user->port=port[1];
  write_user(user,"~OL~FW[** ~FRWizport login ~FW**]\n\n");
  }
...
amsys->num_of_logins++;
    ...
    if (!strncmp(named_site, "fw0.castleaccess.com", 15) ||
		!strncmp(named_site, "dsl-covad-sandg-sn-216188008009", 30) ||
...
void load_and_parse_config(void) {
	...
	perror("Amnuts: Can't open config file");
	boot_exit(1);
    }
/* Main reading loop */
    config_line = 0;
  ...
  /* See if new section */
		...
		section_in = 1;
	    else if (!strcmp(wrd[0], "ROOMS:"))
		section_in = 2;
	    else if (!strcmp(wrd[0], "TOPICS:"))
		section_in = 3;
	    else if (!strcmp(wrd[0], "SITES:"))
		section_in = 4;
	    else {
		...
		break;
	    case 3:
		parse_topics_section(remove_first(line));
		got_topics = 1;
		break;
	    case 4:
#ifdef NETLINKS
...
/* Load room descriptions */
    for(rm1 = room_first; rm1 != NULL; rm1 = rm1->next) {
	sprintf(filename,"%s/%s.R", ROOMDESCS, rm1->name);
	if (!(fp=fopen(filename, "r"))) {
...
void do_events(int sig) {
  ...
  //check_atmos(); //atmospherics kelz
    check_credit_updates(); //money
    reset_alarm();
}
...
/* Use while loop here instead of for loop for when user structure gets
   destructed, we may lose ->next link and crash the program */
	...
	if (!user->afk)
        ...
        if (!user->login) {
    	    if (user->level < ROOT2 && user->level > NEW && user->total_login >= user_level[user->level].promotime)
	        do_promotion(user);
        }
	...
	if (user->warned && tm < amsys->user_idle_time - 60) {
	    user->warned = 0;
	    user = next;
	    continue;
	}
	...
	if (user->login && tm >= amsys->login_idle_time) {
	    write_user(user, "\n\n~OL~FRSystem: ~FW[*** Time out ***]\n\n");
	    disconnect_user(user);
	    user = next;
	    continue;
	}

        if (tm >= amsys->user_idle_time && user->warned) {
	...
	if ((!user->afk || (user->afk && amsys->time_out_afks)) && !user->login && !user->warned && tm >= amsys->user_idle_time - 60) {
...
void do_promotion(UR_OBJECT user) {

    if (user->muzzled && !user->informed) { //check shakle/muzzled
        //your muzzled, you suck
        return;
    } else if (user->lroom == 2 && !user->informed) {
        //your shackled, you suck
        return;
    }

    user->unarrest = user->real_level = ++user->level;
    vwrite_user(user, "~OL~FGYou have been promoted to level ~FW%s%s!!\n",
		user_level[user->level].color, user_level[user->level].name);
    sprintf(text, "~OL~FGAutomatically promoted to next level: ~FW%s\n", 
user_level[user->level].name);
    add_history(user->name, 1, text);

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OL~FGhas been auto-promoted to ~FW%s%s~RS~OL!!\n",
		"~OL[~RS", user->recap, "~RS~OG]", user_level[user->level].color,
		user_level[user->level].name);
        write_level(user->level + 1, 1, 2, text, user); 
	sprintf(text, "%s~RS ~OL~FGhas been auto-promoted to ~FW%s%s~RS~OL!!\n",
		invisname, user_level[user->level].color, user_level[user->level].name);
	write_level(user->level, 0, 2, text, user);
    } else {
	vwrite_room(NULL, "%s~RS ~OL~FGhas been auto-promoted to ~FW%s%s~RS~OL!!\n",
		user->recap, user_level[user->level].color, user_level[user->level].name);
    }
}
  ...
  if (found) {
	...
	for (i=1;i<wcnt;i++)
	  switch(i) {
	    ...
	    case 9: user->ignatmos = atoi(user_words[i]); break;
	    case 10: user->mail_notify = atoi(user_words[i]); break;
         }
	break;
      case 8:
	for (i=1;i<wcnt;i++)
...
int save_user_details(UR_OBJECT user, int save_current) {
	...
	vwrite_user(user, "%s: failed to save your details. Notify an admin right away!!\n",syserror);
...
/* print out the file */
	...
	fprintf(fp, "times         %d %d %d %d\n",
		(int) time(0), (int) user->total_login, (int) (time(0) - user->last_login),
		(int) user->read_mail);
    else
	fprintf(fp, "times         %d %d %d %d\n",
		(int) user->last_login, (int) user->total_login, user->last_login_len,
		(int) user->read_mail);
    fprintf(fp, "levels        %d %d %d %d\n",
		user->level, user->unarrest, user->arrestby, user->muzzled);
    fprintf(fp, "general       %d %d %d %d %d %d %d %d\n",
		user->accreq, user->charmode_echo, user->command_mode, user->prompt,
		user->vis, user->monitor, user->mail_verified, user->logons);
    ...
    fprintf(fp, "user_ignores  %d %d %d %d %d %d %d %d %d %d\n",
		...
		user->ignatmos, user->mail_notify);
...
amsys->user_count=0;
for (i=NEW;i<=BOT;i++) amsys->level_count[i]=0;
/* count up users */
for (entry=first_dir_entry;entry!=NULL;entry=entry->next) {
...
void write_room_level(int level, int above, int record, char *str, UR_OBJECT user, RM_OBJECT room) {
    write_room_level_except(level, above, record, str, user, room, NULL);
}

void write_room_level_except(int level, int above, int record, char *str, UR_OBJECT user, RM_OBJECT room, UR_OBJECT except) {
    UR_OBJECT u;
    ...
    for(u = user_first;u != NULL;u = u->next) {
        ...
        if (u->room != room)
	    continue;
	if ((check_igusers(u, user)) != -1 && user->level < BOT)
            continue;
		...
		if (u->type==CLONE_TYPE) {
		    ...
		    if (room!=u->room || u->owner == user) continue;
...
void write_level_between(int level, int level2, int record, char *str, UR_OBJECT user) { 
    UR_OBJECT u;

    if (abs(level - level2) < 0)
	return;
	...
	if ((check_igusers(u, user)) != -1 && user->level < BOT)
        ...
        if ((u->ignwiz && (com_num == WIZSHOUT || com_num == WIZEMOTE)) || (u->ignlogons && logon_flag))
            continue;
        if (u != user && !u->login) {
	    if ((u->level <= level && u->level > level2) || (u->level >= level && u->level < level2)) {
                if (u->type==CLONE_TYPE) {
                    if (u->clone_hear==CLONE_HEAR_NOTHING || u->owner->ignall) continue;
                    /* Ignore anything not in clones room, eg shouts, system messages
                       and semotes since the clones owner will hear them anyway. */
...
void write_level(int level, int above, int record, char *str, UR_OBJECT user) {
    write_level_except(level, above, record, str, user, NULL);
}

void write_level_except(int level, int above, int record, char *str, UR_OBJECT user, UR_OBJECT except) {
    UR_OBJECT u;
...
/* a vargs wrapper for the write_room_except_both function.  This will enable you
   to send arguments directly to this function
*/
void vwrite_room_except_both(RM_OBJECT rm, UR_OBJECT u1, UR_OBJECT u2, char *str, ...)
{
va_list args;
...
void write_room_except_both(RM_OBJECT rm, char *str, UR_OBJECT u1, UR_OBJECT u2)
{
UR_OBJECT u;
...
for(u=user_first;u!=NULL;u=u->next) {
  if (u->login
      ...
      || (u->ignshouts && com_num==SHOUT)
      || (u->ignshouts && com_num==SEMOTE)
      || (u->ignshouts && com_num==STHINK)
      || (u->ignshouts && com_num==SSING)
      || (u->ignshouts && com_num==USHOUT)
      || (u->ignshouts && com_num==USTHINK)
      || (u->ignshouts && com_num==STHINKTO)
      || (u->ignshouts && com_num==SSINGTO)
      || (u->ignshouts && com_num==SHOUTTO)
  ...
  if (u->type==CLONE_TYPE) {
    ...
    /* Ignore anything not in clones room, eg shouts, system messages
       and semotes since the clones owner will hear them anyway. */
    if (rm!=u->room || u->owner == u1) continue;
    if (u->clone_hear==CLONE_HEAR_SWEARS) {
      if (!contains_swearing(str)) continue;
      }
    sprintf(text2, "~FT[ %s ]:~RS %s", u->room->name, str);
...
for(u=user_first;u!=NULL;u=u->next) {
  if (u->login 
      ...
      || (u->ignall && !force_listen)
      || (u->ignshouts &&
		(com_num==SHOUT || com_num==SEMOTE || com_num == STHINK || com_num == SSING || com_num == USTHINK || com_num == USSING || com_num == SHOUTTO || com_num == SSINGTO || com_num == STHINKTO))
      || (u->ignlogons && logon_flag)
      || (u->igngreets && com_num==GREET)
      || u==user) continue;
  ...
  if (u->type==CLONE_TYPE) {
    ...
    /* Ignore anything not in clones room, eg shouts, system messages
       and semotes since the clones owner will hear them anyway. */
    if (rm!=u->room || u->owner == user) continue;
    if (u->clone_hear==CLONE_HEAR_SWEARS) {
      if (!contains_swearing(str)) continue;
      }
    sprintf(text2,"~FT[ %s ]:~RS %s",u->room->name,str);
...
for(u=user_first;u!=NULL;u=u->next) {
  if (u->login 
      ...
      || (u->ignall && !force_listen)
      || u==user) continue;
  if ((check_igusers(u,user))!=-1 && user->level<BOT) continue;
...
void record_last_command(UR_OBJECT user, int comnum, int len) {
    sprintf(cmd_history[amsys->last_cmd_cnt & 15], "[%5d] %02d/%02d, %02d:%02d:%02d - %-15s - %3d/%-2d - %s",
		amsys->last_cmd_cnt, tmday, tmonth + 1, thour, tmin, tsec,
		command_table[comnum].name, len, word_count, user->name);
    amsys->last_cmd_cnt++;
}
...
switch(user->login) {
  case LOGIN_NAME:
    ...
    for (i=0;i<strlen(name);++i) {
      if (!isalpha(name[i])) {
	write_user(user,"\n~OL~FRSystem: ~FROnly letters are allowed in a name.\n\n");
	attempts(user);  return;
        }
      }
/*
    if (contains_swearing(name)) {
      write_user(user,"\n~OL~FRSystem: ~FWYou cannot use a name like that, sorry!\n\n");
      attempts(user);  return;
      }
*/
    strtolower(name);
    name[0]=toupper(name[0]);
    if (user_banned(name)) {
      write_user(user,"\n~OL~FRSystem: ~FRYou are banned from this talker.\n\n");
      disconnect_user(user);
    ...
    if (!load_user_details(user)) {
      ...
      if (user->port==port[1]) {
	write_user(user,"\n~OL~FRSystem: ~FRSorry, new logins cannot be created on this port.\n\n");
	disconnect_user(user);  
	return;
        }
      if (amsys->minlogin_level>-1) {
	write_user(user,"\n~OL~FRSystem: ~FRSorry, new logins cannot be created at this time.\n\n");
	disconnect_user(user);  
	return;
        }
      if (site_banned(user->site,1)) {
	write_user(user,"\n~OL~FRSystem: ~FRSorry, new accounts from your site have been banned.\n\n");
	write_syslog(SYSLOG,1,"Attempted login by a new user from banned new users site %s.\n",user->site);
	disconnect_user(user);
	return;
        }
      write_user(user,"\n\n~OL~FW~LIAba!!! ~FGNew User...\n\n");
      }
    else {
      if (user->port==port[1] && user->level<amsys->wizport_level) {
	vwrite_user(user,"\n~OL~FRSystem: ~FWSorry, only users of level %s and above can log in on this port.\n\n",user_level[amsys->wizport_level].name);
	disconnect_user(user);  
	return;
        }
    ...
    write_user(user,"Give me a password:~RS~FK ");
    echo_off(user);
  ...
  case LOGIN_PASSWD:
    ...
    if (strlen(passwd)<3) {
      write_user(user,"\n\n~OL~FRSystem: ~FRPassword too short.\n\n");  
      attempts(user);  return;
      }
    if (strlen(passwd)>PASS_LEN) {
      write_user(user,"\n\n~OL~FRSystem: ~FRPassword too long.\n\n");
      attempts(user);  return;
      }
    /* if new user... */
    if (!user->pass[0]) {
	...
	write_user(user,"\n~OL~FTBy typing your password in again you are accepting the above rules.\n");
	write_user(user,"~OL~FTIf you do not agree with the rules, then disconnect now.\n");
        }
      write_user(user,"\nGive me a password:~RS~FK ");
  ...
  case LOGIN_CONFIRM:
    sscanf(inpstr,"%s",passwd);
    if (strcmp(user->pass,(char*)crypt(passwd,crypt_salt))) {
      write_user(user,"\n\n~OL~FRSystem: ~FWPasswords do not match.\n\n");
      attempts(user);
      return;
      }
    echo_on(user);
    strcpy(user->desc,"~OLa ~FRnewbie~FW.");
    strcpy(user->in_phrase,"enters");	
    strcpy(user->out_phrase,"goes");	
    strcpy(user->date,(long_date(1)));
...
if (user->attempts==LOGIN_ATTEMPTS) {
  write_user(user,"\n~OL~FRSystem: ~OL~FWMaximum attempts reached.\n\n");
  disconnect_user(user);
  return;
  }
reset_user(user);
...
sprintf(text,"You're joining us %s",long_date(1));
cnt=colour_com_count(text);
vwrite_user(user,"| %-*.*s |\n",74+cnt*3,74+cnt*3,text);
write_user(user,"|----------------------------------------------------------------------------|\n");
/*
if (user->last_site[0]) {
  ...
  sprintf(text,"Last login date : ~FW%-25s~RS  You are level: %s%s~RS", temp, user_level[user->level].color, user_level[user->level].name);
  cnt=colour_com_count(text);
  vwrite_user(user,"| %-*.*s |\n",74+cnt*3,74+cnt*3,text);
  sprintf(text,"Last login site : ~FW%s~RS",user->last_site);
  cnt=colour_com_count(text);
  vwrite_user(user,"| %-*.*s |\n",74+cnt*3,74+cnt*3,text);
  exline++;
  }
...
if (user->level>=command_table[INVIS].level) {
  ...
  if (user->level>=command_table[MONITOR].level) {
    sprintf(text2,"    Your monitor is %s~RS",myoffon[user->monitor]);
    strcat(text,text2);
    }
  else strcat(text,"\n");
  cnt=colour_com_count(text);
...
if (yes) {
  sprintf(text,"You are ignoring: ~OL%s~RS",text2);
  cnt=colour_com_count(text);
  vwrite_user(user,"| %-*.*s |\n",74+cnt*3,74+cnt*3,text);
  exline++;
  }
...
if (user->level>=command_table[REMINDER].level) {
  ...
  if (cnt) {
    sprintf(text,"There %s %d reminder%s removed due to date being passed",PLTEXT_WAS(cnt),cnt,PLTEXT_S(cnt));
    vwrite_user(user,"| %-80.80s |\n",text);
    }
  write_user(user,"|----------------------------------------------------------------------------|\n");
  }
...
void connect_user(UR_OBJECT user) {
	...
	if (user != u && user->type != CLONE_TYPE && !strcmp(user->name, u->name)) {
	    ...
	    if (u->type == REMOTE_TYPE) {
	        write_user(u, "\n~OLYou are pulled back through cyberspace...\n");
...
#endif
	    write_user(user, "~OL~FYYou are already connected - switching to old session...\n");
	    write_user(u, "~FRPerforming a session swap...\n");
	    write_syslog(SYSLOG, 1, "%s ~OL~FYswapped sessions (%s)\n", user->name, user->site);
	    close(u->socket);
	    u->socket = user->socket;
	    if (!user->vis) {
	        sprintf(text, "~OL[~RS~OLReconnect INVIS~RS~OL]->~RS %s%s%s~RS %s\n", "~OL[~RS", user->recap, "~RS~OG]~RS", user->desc);
		if (user->level == BOT)
		    write_level(BOT, 1, 0, text, NULL);
		else
		    write_level(user->level + 1, 1, 0, text, NULL);
	    } else {
		sprintf(text, "~OL[~RS~OLReconnect~RS~OL]->~RS %s~RS %s\n", user->recap, user->desc);
    ...
    if (!user->vis && user->level < command_table[INVIS].level)
	user->vis = 1;

    if (user->level == NEW) {
        user->room = get_room(default_new);
        if (user->total_login >= 60)
	    user->room = get_room(default_warp);
	if (user->room == NULL) {
	    user->room = room_first;
        }
	...
	rmname[0] = '\0';
    else if (user->room->access == PERSONAL_LOCKED || user->room->access == PERSONAL_UNLOCKED)
	sprintf(rmname, "~RS %s~OL", user->room->name);
    else
	sprintf(rmname, "~RS (%s)~OL", user->room->name);
    if (user->level == NEW) {
	sprintf(text, "\07%s %s~RS %s~RS ~OL[~OL%s~RS~OL]\n",
		JLOGIN, user->recap, user->desc, user->room->name);
        write_room(NULL, text);
    } else {
	if (user->vis) {
	    sprintf(text, "\07%s %s~RS %s~RS ~OL[~OL%s~RS~OL]\n",
		LOGIN, user->recap, user->desc, user->room->name);
            write_room(NULL, text);
        }  else {
    	    sprintf(text, "\07%s %s~RS %s~RS ~OL[~OL%s~RS~OL]\n",
		ILOGIN, user->recap, user->desc, user->room->name);
            if (user->level == BOT)
                write_level(BOT, 1, 0, text, NULL);
            else
  	        write_level(user->level + 1, 1, 0, text, NULL);
	...
	user->t_expire = time(0) + (USER_EXPIRES * 86400);
    user->last_login = time(0); /* set to now */
    look(user);
    /* show how much mail the user has */
    newmail = mail_sizes(user->name, 1);
if (newmail) vwrite_user(user,"\07~FT~OL*** YOU HAVE ~RS~OL%d~FT UNREAD MAIL MESSAGE%s ***\n",newmail,newmail==1?"":"S");
...
write_user(user,"\n~OL~FY---------------------------------------------------------------------------\n");
  write_user(user,"~OL~FY               ~FRTO BE AUTO-PROMOTED PLEASE READ CAREFULLY~OL                  |\n");
  write_user(user,"~OL~FY   ~FTYou must set your description (.desc), set your gender (.gender) and   ~OL|\n");
  write_user(user,"~OL~FY   ~FTuse the .accreq command - once you do all these you will be promoted   ~OL|\n");
...
amsys->num_of_users++;
amsys->num_of_logins--;
if (!(strcmp(user->name,"Kelotz"))    && user->vis) bot_trigger(user,"coderlog",1);
else if (user->vis) 
bot_trigger(user,"login!@#$%^&*()greet)(*&^%$#@!nigol",1);
}
...
write_user(user,"~OL~FB       NAME                     ADDRESS                  PORT\n");
write_user(user,"~OL~FT|-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-|\n");
write_user(user,"~OL~FW       Shell Matrix             chat.shell-matrix.com    2223\n");
write_user(user,"~OL~FT|-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-|\n");
write_user(user,"\n");
  write_user(user,"\n~OL~FYThanks for visiting ~RS~OL~BB[ Chat Manila ]~RS ~OL~FYCome back again!\n\n");
  ...
  logon_flag=1;
  if (user->vis) {
        sprintf(text, "\07%s %s~RS %s~RS\n", LOGOUT, user->recap, user->desc);
        write_room(NULL, text);
  } else {
        sprintf(text, "\07%s %s~RS %s~RS\n", ILOGOUT, user->recap, user->desc);
        write_level(user->level + 1, 1, 0, text, NULL);
    }
  logon_flag=0;
#ifdef NETLINKS
...
*/
  ...
  case '+':
    ...
    break;
  case '-':
    if (isspace(inpstr[1])) strcpy(word[0],"secho");
    else { split_command_string(inpstr);  strcpy(word[0],"secho"); }
    break;
  case '\'':
...
*/
  ...
  case '*':
    strcpy(word[0],"clroom");  return 2;
  case '[':
    if (isspace(inpstr[1])) strcpy(word[0], "twiz");
    else { split_command_string(inpstr);  strcpy(word[0],"twiz"); }
    break;
  case ']':
    if (isspace(inpstr[1])) strcpy(word[0], "ewiz");
    else { split_command_string(inpstr);  strcpy(word[0], "ewiz"); }
    break;
...
int exec_com(UR_OBJECT user, char *inpstr) {
    ...
    if (!comword[0]) {
	write_user(user, "~OL~FRSystem: ~FWUnknown Command.\n");
	return 0;
    }
	...
	if (!strncmp(command_table[i].name, comword, len)) {
	    if (len < 2 && i == QUIT) {
	        com_num = -2;
	        break;
	    }
    ...
    i = 0;
    if (com_num == -1) {
        if (!social(user,inpstr) || user->level < USER) {
            write_user(user, "~OL~FRSystem: ~FWUnknown Command.\n");
    	    return 0;
        }
        return 0;
    ...
    if (com_num == -2) {
        write_user(user, "~OL~FRSystem: ~FWYou need to type at least '~FYqu~FW' to quit.\n");
        return 0;
    }

    //record_last_command(user, com_num, strlen(inpstr)); //was comented out, not sure why
...
//    if (com_num < 1000) {
	    ...
	    if (cmd->id == com_num) {
	        ...
	        if (user->room != NULL && (has_xcom(user, cmd->id))) {
	            write_user(user, "~OL~FRSystem: ~FWYou cannot currently use that command.\n");
		    return 0;
            	}
	        if (user->room!=NULL && (cmd->min_lev > user->level)) {
	            vwrite_user(user, "~OL~FRSystem: ~FWThat command is for level ~FT%s%s~FW and above.\n",
		           	user_level[cmd->min_lev].color, user_level[cmd->min_lev].name);
		    return 0;
            	}
	    	++cmd->count;
	    	break;
...
//    }
	...
	switch(com_num) {
	    ...
	    case SUICIDE: 
	    case CHARECHO:
	         write_user(user, "~OL*** Home execution ***\n");
	         break;
	    default:
...
//    if (com_num < 1000) {
	...
	switch(com_num) {
	    case QUIT:
		if (strlen(word[0]) < 2)
		    write_user(user, "~OL~FRSyetem: ~FWYou need to type at least '~FYqu~FW' to quit.\n");
		else
		    disconnect_user(user);
		break;
	    ...
	    case USHOUT  :
	    case SHOUTTO :    shout_to(user, inpstr); break;
	    case NSHOUT  :    nshout(user, inpstr); break;
            case TELL    :    tell(user, inpstr);   break;
            case REPLY   :    reply(user, inpstr); break;
            case WIZTELL :    wiz_tell(user, inpstr); break;
            ...
            case CSITE   :    csite(user,inpstr); break;
            case CLSALL  :    cls_all(user); break;
            case ASSASIN :    assasin_user(user);  break;
            case SECHO   :    shout_echo(user, inpstr); break;
	    ...
	    case PUBCOM  :
            ...
            case COMMANDS:    help_commands_level(user, 0); break;
            ...
            case SBAN    :    toggle_shoutban(user); break;
            ...
            case GENDER  :    set_attributes(user, inpstr, 2);  break;
            case RECAP   :    set_attributes(user, inpstr, 1);  break;
            case SET     :    set_attributes(user, inpstr, 0);  break;
	    ...
	    case UTHINK  :    think_to(user, inpstr); break;
            case STHINK  :    sthink(user, inpstr); break;
            case USTHINK :    sthink_to(user, inpstr); break;
            case STHINKTO:    sthink_to(user, inpstr); break;
            case SINGIT  :    sing_it(user, inpstr);  break;
	    case USING   :    sing_to(user, inpstr); break;
            case SSING   :    ssing(user, inpstr); break;
            case USSING  :    ssing_to(user, inpstr); break;
            case SSINGTO :    ssing_to(user, inpstr); break;
            case SOCIALS :    list_socials(user, 0); break;
            ...
            case ARREST  :    arrest(user, inpstr);  break;
		...
		clear_edit(user);  
            ...
            case SETAUTOPROMO: 
                switch(amsys->auto_promote) {
                    case 0:
			write_user(user, "You have turned ~FGon~RS auto-promotes for new users.\n");
	          	amsys->auto_promote = 1;
        	  	write_syslog(SYSLOG, 1, "%s turned ON auto-promotes.\n", user->name);
          		break;
                    case 1:
			write_user(user, "You have turned ~FRoff~RS auto-promotes for new users.\n");
       	   		amsys->auto_promote = 0;
          		write_syslog(SYSLOG, 1, "%s turned OFF auto-promotes.\n", user->name);
          		break;
                }
              break;
            case USAY    :    say_to(user, inpstr);  break;
  ...
  case URL:
    sprintf(filename,"%s/%s",MISCFILES,URLFILE);
    switch(more(user,user->socket,filename)) {
      case 0: write_user(user,"\n~OL~FR:System: ~FWThere are currrently no talkers in the list.\n");  break;
      case 1: user->misc_op=2;
      }    
    break;
  case CRITERIA:
    sprintf(filename,"%s/%s",MISCFILES,CRITERIAFILE);
    switch(more(user,user->socket,filename)) {
      case 0: write_user(user,"\n~OL~FRSystem: ~FWThere are currrently no criteria in the list.\n");  break;
      case 1: user->misc_op=2;
      }
    break;
          ...
          //add commands here please kelz
          //case ATMOS   :    edit_atmos(user, inpstr); break;
            case IGNATMOS:    set_ignore(user); break;
            case TLOCK   :    topic_lock(user); break;
            case CLRWIZ  :    clear_wiz(); break;
	...
	// money
	    ...
	    case BANK    :    bank_money(user);  break;
          
	    case TAXIUSER:    taxi_user(user, inpstr); break;
	    case WATCHUSER:   watchuser(user, inpstr); break;
...
/*
	...
	switch(com_num - 1000) {
	    ...
	    case BSPEC:
		//buy special items...
		write_user(user, "Not active yet.\n");
		break;
	    case TCLONE:
		create_clone(user);
		break;
		...
		break;
	    case HDESC:
		hack_desc(user, inpstr);
		break;
	    case HBANK:
		hack_bank(user);
		break;
	    case HDISCO:
		hack_disconnect(user);
		break;
	    default:
		write_user(user, "Career command not executed.\n");
	}
    }
*/
    return 1;
...
for(i = 0; i < user->pager; ++i) write_user(user,"\n");		
}
...
if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, you cannot mail anyone.\n");  return;
  }
if (done_editing) {
  if (*user->malloc_end--!='\n') *user->malloc_end--='\n';
...
SKIP:
fclose(fp);
vwrite_user(user,"\nMail message number ~FW~OL%d~RS out of ~FW~OL%d~RS.\n\n",smail_number,total);
}
...
if (done_editing) {
  ...
  write_syslog(SYSLOG,1,"%s sent mail to all the %s's.\n",user->name,user_level[user->lmail_lev].name);
  user->lmail_lev=-3;
  return;
  }
if (word_count>2) {
...
else sprintf(text,"\n~FG*** Writing broadcast level mail message to all the %s's ***\n\n",user_level[user->lmail_lev].name);
write_user(user,text);
user->misc_op=9;
editor(user,NULL);
}
...
fclose(fpo);
send_forward_email(user->email,filenameo);
vwrite_user(user,"You have now sent smail number ~FW~OL%d~RS to your email account.\n",smail_number);
}
...
SKIP:
fclose(fp);
vwrite_user(user,"\nMessage number ~FW~OL%d~RS out of ~FW~OL%d~RS.\n\n",msg_number,rm->mesg_cnt);
if (user->vis) name=user->recap;  else name=invisname;
if (rm==user->room)
  if (user->level<BOT || user->vis) vwrite_room_except(user->room,user,"%s~RS reads the message board.\n",name);
}
...
if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, you cannot write on the board.\n");  
  return;
  }
if (!done_editing) {
  if (word_count<2) {
...
vwrite_room_except(user->room, user, "%s~RS writes a message on the board.\n", (user->vis) ? user->recap : invisname);
user->room->mesg_cnt++;
}
...
switch(is_personal_room(rm)) {
  case 0:
    ...
    break;
  case 1:
    if (!is_my_room(user,rm) && user->level<BOT && !check_board_wipe(user)) return;
    else if (get_wipe_parameters(user)==-1) return;
    break;
  }
if (user->vis) name=user->recap; else name=invisname;
...
if (user->wipe_from==-1) {
  ...
  if (user->level<BOT || user->vis) vwrite_room_except(rm,user,"%s~RS wipes the message board.\n",name);
...
if (cnt==rm->mesg_cnt) {
  ...
  if (user->level<BOT || user->vis) vwrite_room_except(rm,user,"%s wipes the message board.\n",name);
  write_syslog(SYSLOG,1,"%s wiped all messages from the board in the %s.\n",user->name,rm->name);
  rm->mesg_cnt=0;
  return;
  }
...
if (user->level<BOT || user->vis) vwrite_room_except(rm,user,"%s wipes some messages from the board.\n",name);
...
void look(UR_OBJECT user) {
    RM_OBJECT rm;
    UR_OBJECT u;
    char temp[81], null[1], *ptr, *ptr2;
    char *uafk= "~FR(AFK)~RS", *uedit = "~FT(EDIT)~RS";
    int i, exits, users;
	...
	if (rm->link[i] == NULL)
	    ...
	    sprintf(temp, "  ~FR%s~RS", rm->link[i]->name);
	else
...
for(u=user_first;u!=NULL;u=u->next) {
  ...
  if (!users++) write_user(user,"~FGYou can see:\n");
  if (u->afk) ptr=uafk; else ptr=null;
  if (u->editing) ptr2 = uedit; else ptr2 = null;
  if (!u->vis) vwrite_user(user,"     ~FR*~RS%s~RS %s~RS  %s%s\n",u->recap,u->desc,ptr, ptr2);
  else vwrite_user(user,"      %s~RS %s~RS  %s%s\n",u->recap,u->desc,ptr, ptr2);
  }
...
sprintf(temp," and there %s ~OL%d~RS message%s on the board.\n",PLTEXT_IS(rm->mesg_cnt),rm->mesg_cnt,PLTEXT_S(rm->mesg_cnt));
strcat(text,temp);
write_user(user,text);
if (rm->topic[0]) vwrite_user(user,"~OLTopic: ~RS%s~RS\n", rm->topic);
else write_user(user,"No topic has been set.\n");	
    if (rm->topic[0] && rm->tlock)
	vwrite_user(user, "Locked at level: %s%s\n", user_level[rm->tlock].color, user_level[rm->tlock].name);
}
...
void who(UR_OBJECT user, int type) {
	...
	write_user(user, "\n|----------------------------------------------------------------------------|\n");
	write_user(user, "| ~OLUser login stages are as follows~RS                                           |\n");
	write_user(user, "|----------------------------------------------------------------------------|\n");
	vwrite_user(user, "| ~OLStage %d :~RS The user has logged onto the port and is entering their name     |\n",LOGIN_NAME);
	vwrite_user(user, "| ~OLStage %d :~RS The user is entering their password for the first time           |\n",LOGIN_PASSWD);
	vwrite_user(user, "| ~OLStage %d :~RS The user is new and has been asked to confirm their password     |\n",LOGIN_CONFIRM);
	vwrite_user(user, "| ~OLStage %d :~RS The user has entered the pre-login information prompt            |\n",LOGIN_PROMPT);
	write_user(user, "|----------------------------------------------------------------------------|\n\n");
	return;
    }
    ...
    write_user(user, "                         ~OL~FW ~FWChat Manila~FW~RS \n");
    write_user(user, LINEA);
    write_user(user, center_string(78, 0, NULL, "%sCurrent users %s~RS", (user->login) ? "": "~BB~FW", long_date(1)));
    switch(type) {
	case 0:
	    vwrite_user(user, "%s  Name                                        Rank     :S: Room       : Tm/Id\n",
			(user->login) ? "": "~OL~FW");
	    break;

	case 1:
	    write_user(user, "  ~FTFriend                                      Rank     :S: Room       : Tm/Id\n");
	    break;
    ...
    write_user(user, (user->login) ? LINEA : LINEA);
    for (u = user_first; u != NULL; u = u->next) {
	if (u->type == CLONE_TYPE) 
	    continue;
	...
	if (!u->vis) { 
	    ++invis;  
	    if (u->level > user->level && !(user->level >= ROOT2))
		continue;  
	}

	if (type == 2) {
	    if (u->type==BOT_TYPE) sprintf(idlestr," ~FRBOT~RS");
	    else	
	    if (u->afk)
		strcpy(idlestr, " ~FRAFK~RS");
	    else if (u->editing)
		strcpy(idlestr, "~FTEDIT~RS");
	    else
	    ...
	    vwrite_user(user, "%-15s : %s%-5.5s~RS   %s  %s  %s %s %4d  %s  %s\n",
			u->name, user_level[u->level].color, user_level[u->level].name, sockstr, noyes1[u->ignall], noyes1[u->vis],
			idlestr, mins, portstr, u->site);
	    continue;
	}
	...
	if (u->afk)
		...
		strcpy(idlestr, "~FTEDIT~RS");
	else if (idle >= (amsys->user_idle_time / 60))
		strcpy(idlestr, "~FYIDLE~RS");
	else
		sprintf(idlestr, "~FW%d~OL/~RS~FW%d~RS", mins, idle);

	sprintf(text, " %-*.*s~RS %s%+15s~RS :%s~RS: ~OL%-11.11s~RS: %s\n",
			37+cnt*3, 37+cnt*3, line, user_level[u->level].color, user_level[u->level].name, sex[u->gender], rname, idlestr);
	if (user->login)
	    write_user(user, colour_com_strip(text));
	else
	    write_user(user, text);
    }
	...
	case 2:
#ifdef NETLINKS
	    sprintf(text, "Total of ~OL%d~RS user%s: ~OL%d~RS visible, ~OL%d~RS invisible, ~OL%d~RS remote",
			total, PLTEXT_S(total), total - invis, invis, remote);
#else
	    sprintf(text, "~FT%d~RS total user%s logged in: ~FT%d~RS visible, ~FT%d~RS invisible",
			total, PLTEXT_S(total), total - invis, invis);
#endif
	    break;

	case 1:
	    sprintf(text, "Total of ~OL%d~RS friend%s : ~OL%d~RS visible, ~OL%d~RS invisible",
			total, PLTEXT_S(total), total - invis, invis);
	    break;
    }

    if (user->login)
        write_user(user, center_string(78, 0, NULL, colour_com_strip(text)));
    else
       write_user(user, center_string(78, 0, NULL, text));
...
if (!is_wrap) {
  ...
  write_user(user, LINEA);
  sprintf(text,"~OL~FGCommands available for level ~FG%s%s~RS",user_level[user->level].color, user_level[user->level].name);
  sprintf(text2,"| %-89s|\n",text);
  write_user(user,text2);
  vwrite_user(user,"%s\n", LINEA);
  /* set some defaults */
...
if (!is_wrap) {
  ...
  write_user(user,"| Remember, a '.' by itself will repeat your last command or speech          |\n");
  write_user(user,"|----------------------------------------------------------------------------|\n");
  sprintf(text,"~FT~OLCommands available for level ~RS%s~RS",user_level[user->level].color, user_level[user->level].name);
  sprintf(text2,"| %-89s |\n",text);
  write_user(user,text2);
  write_user(user,"|----------------------------------------------------------------------------|\n\n");
  /* set some defaults */
...
write_user(user, "\n~FRMasta~FYTalka~RS, Modifidied AMNUTS, Copyright (C) Michael Berg, 2002\n");
write_user(user, "I don't have a whole lot to say, really, just this: my mods are free. Any\n");
write_user(user, "If you have questions or want the code, e-mail me: mastamike@mac.com.\n");
write_user(user, "Thanx: Kokopelli for getting me into this, you bastard, and to Lestat, Orson,\n");
write_user(user, "and I belive I owe more than half of the socials to Kain, and to the OG crew\n");
write_user(user, "of ~OLThe ~OLTourist ~OLTrap~RS for the endless hours of forced beta testing ;).\n");
write_user(user, "~FRMasta~FYMike~RS, July 2002\n\n");
write_user(user,"\n~BM             ~BB             ~BT             ~BG             ~BY             ~BR             ~RS\n\n");
write_user(user,"Enhanced by Matrix Coders: Backslash/Ranma/Junior/Foxfire/Trigger v2.1.1\n");
write_user(user,"System Compiled By: Jhong\n"); 
}
...
void say(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot speak.\n");
	return;
    }
    if (amsys->ban_swearing && contains_swearing(inpstr)) {
	switch(amsys->ban_swearing) {
	    case SBMIN:
		if (!(in_private_room(user)))
	    ...
	    default:
		break; /* do nothing as ban_swearing is off */
	}
    }
    ...
    if (user->type == CLONE_TYPE) {
	sprintf(text, "~OLClone~RS~FT of %s~RS ~FT%ss:~RS %s\n", user->recap, type, inpstr);
	vwrite_room(user->room, text);
	record(user->room, text);
	return;
    }

    vwrite_user(user, "~OL~FWYou %s:~RS %s\n", type, inpstr);
    if (user->vis) name=user->bw_recap; else name=invisname; 
    if (!user->vis) write_monitor(user,user->room,0);
    sprintf(text, "~RS~OL~FG[~FW%s~RS~OL~FG]~FW:~RS %s\n", name, inpstr);
    write_room_except(user->room, text, user);
    record(user->room, text);
    bot_trigger(user,inpstr,0);

    }

csite(UR_OBJECT user, char *inpstr)      
{
if (word_count<2) {
  write_user(user,"Usage: csite <new site>\n");  return;
  }
strcpy(user->site,word[1]);
sprintf(text,"~OL~FTYour site has been changed to: ~FR%s\n",user->site);
write_user(user,text);
}
...
{
if (user->muzzled) {     
  write_user(user,"~OL~FRSystem: ~FWYou are muzzled, you cannot clear the screen.\n");
  return;
  }

clear_revbuff(user->room);   
write_room(user->room,"\033[H\033[J");
}



/*** Assasin a user off the talker, make it look like .q (Felix) ***/
void assasin_user(UR_OBJECT user)      
{
UR_OBJECT victim;
char *name;

if (word_count<2) {
  write_user(user,"Usage:  assasin <user>\n");
  return;
  }
...
if (user==victim) {
  write_user(user,"Try .q.\n");
  return;
  }
if (victim->level>=user->level) {
  write_user(user,"~OL~FRSystem: ~FWYou cannot assasin a user of higher level than yourself.\n");
  return;
  }
sprintf(text,"%s assasined %s.\n",user->name,victim->name);
write_syslog(SYSLOG,1,text);
sprintf(text,"Assasined by %s.\n",user->name);
...
void say_to(UR_OBJECT user, char *inpstr) {
    ...
    if ((word_count < 3 && inpstr[0] != '-') || (word_count < 2 && inpstr[0] == '-')) {
    ...
    if (u == user) {
        write_user(user, "You can't do that, we dont want you to look crazy.\n");
        return;
    }

    if (user->room != u->room) {
	write_user(user, "You must to be in the same room as them to do that.\n");
	return;
    }
    ...
    if (strncmp(type, "ask", 3) == 0) {
        vwrite_user(user, "~FTYou %s ~RS%s%s%s~FT:~RS %s\n",
		type, (u->vis) ? "" : "~OL[~RS", name2, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
	vwrite_user(u, "%s%s%s ~FT%ss you:~RS %s\n",
		(user->vis) ? "" : "~OL[~RS", name1, (user->vis) ? "" : "~RS~OG]~RS", type, inpstr); 
    } else {
        vwrite_user(user, "~FTYou %s to ~RS%s%s%s~FT:~RS %s\n",
		type, (u->vis) ? "" : "~OL[~RS", name2, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
        vwrite_user(u, "%s%s%s ~FT%ss to you:~RS %s\n", 
                (user->vis) ? "" : "~OL[~RS", name1, (user->vis) ? "" : "~RS~OG]~RS", type, inpstr); 
    }

    if (!user->vis) {
	if (strncmp(type, "ask", 3) == 0) {
            sprintf(text, "%s%s%s~RS ~FT%ss ~RS",
			"~OL[~RS", name1, "~RS~OG]", type);
	} else {
	    sprintf(text, "%s%s%s~RS ~FT%ss to ~RS",
			"~OL[~RS", name1, "~RS~OG]", type);
	}
	write_room_level_except(user->level + 1, 1, 0, text, user, user->room, u);
	name1 = invisname;
        if (strncmp(type, "ask", 3) == 0) {
            sprintf(text, "%s~RS ~FT%ss ~RS", name1, type);
        } else {
            sprintf(text, "%s~RS ~FT%ss to ~RS", name1, type);
	}
	write_room_level_except(user->level, 0, 0, text, user, user->room, u);
    } else {
	if (strncmp(type, "ask", 3) == 0)
            sprintf(text, "%s~RS ~FT%ss ~RS", name1, type);
	else
    ...
    if (!u->vis) {
        sprintf(text, "%s%s%s~RS~FT:~RS %s\n", "~OL[~RS", name2, "~RS~OG]", inpstr);
	write_room_level_except(u->level + 1, 1, 0, text, user, user->room, u);
	name2 = invisname;
	sprintf(text, "%s~RS~FT:~RS %s\n", name2, inpstr);
	write_room_level_except(u->level, 0, 0, text, user, user->room, u);
    } else {
	sprintf(text, "%s~RS~FT:~RS %s\n", name2, inpstr);
        write_room_except_both(user->room, text, user, u);
    }
...
void shout(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot shout.\n");
	return;
    }
    ...
    if (amsys->shout_ban && user->level < BOT) {
        write_user(user, "~FRALL shouted commands have been disabled due to abuse!\n");
        return;
    }
    ...
    if (!user->vis) {
	sprintf(text, "%s%s%s~RS ~OLshouts:~RS %s\n", "~OL[~RS", name, "~RS~OG]", inpstr);
	write_level(user->level + 1, 1, 0, text, user);
	name = invisname;
	sprintf(text, "%s~RS ~OLshouts:~RS %s\n", name, inpstr);
	write_level(user->level, 0, 0, text, user);
    } else {
	sprintf(text, "%s~RS ~OLshouts:~RS %s\n", name, inpstr);
...
void shout_to(UR_OBJECT user, char *inpstr) {
    char type[10], *name1, *name2, *temp;
    UR_OBJECT u;

    if (word_count < 3) {
        write_user(user,"Usage: ushout <person> <message>\n");
        return;
    }
    ...
    if (u == user) {
	write_user(user, "Stop acting crazy, you'll scare the locals.\n");
	return;
    }
    ...
    vwrite_user(user, "~OLYou shout to ~RS%s%s%s~RS~OL:~RS %s\n",
		(u->vis) ? "" : "~OL[~RS", name2, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
    vwrite_user(u, "%s%s%s~RS ~OLshouts to you: ~RS%s\n",
		(user->vis) ? "" : "~OL[~RS", name1, (user->vis) ? "" : "~RS~OG]", inpstr);

    if (!user->vis) {
	sprintf(text, "%s%s%s~RS ~OLshouts to ~RS", "~OL[~RS", name1, "~RS~OG]");
	write_level_except(user->level + 1, 1, 0, text, user, u);
	name1 = invisname;
	sprintf(text, "%s~RS ~OLshouts to ~RS", name1);
	write_level_except(user->level, 0, 0, text, user, u);
    } else {
	sprintf(text, "%s~RS ~OLshouts to ~RS", name1);
	write_room_except_both(NULL, text, user, u);
    }

    record_shout(text);

    if (!u->vis) {
	sprintf(text, "%s%s%s~RS~OL:~RS %s\n", "~OL[~RS", name2, "~RS~OG]", inpstr);
	write_level_except(u->level + 1, 1, 0, text, user, u);
	name2 = invisname;
	sprintf(text, "%s~RS~OL:~RS %s\n", name2, inpstr);
	write_level_except(u->level, 0, 0, text, user, u);
    } else {
	sprintf(text, "%s~RS~OL:~RS %s\n", name2, inpstr);
	write_room_except_both(NULL, text, user, u);
    }
...
void nshout(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    char *name, *name2;

    if (word_count < 3) {
        write_user(user,"Usage: nshout <name> <message>\n");
        return;
    }
    ...
    if (u == user) {
        write_user(user, "Um, how on earth do you think you can accomplish that?\n");
        return;
    }

    vwrite_user(user, "~OLYou shout to everyone except ~RS%s~RS~OL:~RS %s\n", u->recap, inpstr);
    name = user->recap;

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OLshouts to everyone except ~RS", "~OL[~RS", name, "~RS~OG]");
        write_level_except(user->level + 1, 1, 0, text, user, u);
        name = invisname;
        sprintf(text, "%s~RS ~OLshouts to everyone except ~RS", name);
        write_level_except(user->level, 0, 0, text, user, u);
    } else {
        sprintf(text, "%s~RS ~OLshouts to everyone except ~RS", name);
...
void tell(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot tell anyone anything.\n");
	return;
    }
...
/* if .t <u> with no message */
	if (word_count < 3) {
	    write_user(user, "Tell them what?\n");
	    return;
	}
    ...
    if (u == user) {
	write_user(user,"~OL~FWUm, if thats what you really want, you don't even have to type it.\n");
	return;
    }
    ...
    if (!user->vis)
	sprintf(text, "%s%s%s~RS~OL %ss you:~RS %s\n", "~OL[~RS", name, "~RS~OG]", type, inpstr);
    else
	sprintf(text, "%s~RS~OL %ss you:~RS %s\n", name, type, inpstr);

    if (u->afk) {
	if (u->afk_mesg[0])
	    vwrite_user(user, "%s~RS is ~FRAFK~RS: %s\n", u->recap, u->afk_mesg);
	else
    ...
    if (!u->vis)
	sprintf(text, "~OLYou %s ~RS%s%s%s~RS~OL:~RS %s\n", type, "~OL[~RS", name, "~RS~OG]", inpstr);
    else
	sprintf(text, "~OLYou %s ~RS%s~OL:~RS %s\n", type, name, inpstr);

    write_user(user,text);
    strcpy(u->replyto, user->name);
...
void reply(UR_OBJECT user, char *inpstr) {
    ...
    if (strlen(user->replyto) < 3) {
	write_user(user, "You have no one to reply to.\n");
	return;
    }

    u = get_user_name(user, user->replyto);
    ...
    if (u == user) {
        write_user(user,"Um, if thats what you really want, you don't even have to type it.\n");
        return;
    }
...
void wiz_tell (UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    char type[5], *name;
    ...
    if (word_count < 2) {
        write_user(user, "Wiz tell them what?\n");
        return;
    }
    ...
    if (!user->vis)
        sprintf(text, "~OL~LIWiz~RS %s%s%s~RS ~OLinforms you:~RS %s\n", "~OL[~RS", name, "~RS~OG]", inpstr);
    else
	sprintf(text, "~OL~LIWiz~RS %s~RS ~OLinforms you:~RS %s\n", name, inpstr);
/*
    if (u->afk) {
        if (u->afk_mesg[0])
            vwrite_user(user, "%s~RS is ~FRAFK~RS: %s\n", u->recap, u->afk_mesg);
...
*/  //i want this message to appear to 'u' even if they are afk
    ...
    if (!u->vis)
        sprintf(text, "~OLYou wiz tell ~RS%s%s%s~RS~OL:~RS %s\n", "~OL[~RS", name, "~RS~OG]", inpstr);
    else
        sprintf(text, "~OLYou wiz tell ~RS%s~OL:~RS %s\n", name, inpstr);
...
void think_it(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
        write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot use .think.\n");
        return;
    }
    ...
    vwrite_user(user, "~FRYou think . o O ( ~RS%s~RS~FR )\n", inpstr);
    name = user->recap;

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~FRthinks . o O ( ~RS%s~RS~FR )\n", "~OL[~RS", name, "~RS~OG]", inpstr);
        write_room_level(user->level + 1, 1, 0, text, user, user->room);
        name = invisname;
        sprintf(text, "%s~RS ~FRthinks . o O ( ~RS%s~RS~FR )\n", name, inpstr);
        write_room_level(user->level, 0, 0, text, user, user->room);
    } else {
        sprintf(text, "%s~RS ~FRthinks . o O ( ~RS%s~RS~FR )\n", name, inpstr);
        write_room_except(user->room, text, user);
    }
...
void think_to(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    char *name1, *name2;

    if (user->muzzled) {
        write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot think to anyone.\n");
        return;
    }

    if (word_count < 3) {
        write_user(user, "Think what to who?\n");
        return;
    }
    ...
    if (user->room != u->room) {
        write_user(user, "You must to be in the same room as them to do that, try .usthink.\n");
        return;
    }
    ...
    vwrite_user(user, "~FRYou think to ~RS%s%s%s~FR . o O (~RS %s~RS~FR )\n",
                (u->vis) ? "" : "~OL[~RS", u->recap, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
    vwrite_user(u, "%s%s%s ~FRthinks to you . o O (~RS %s~RS~FR )\n",
                (user->vis) ? "" : "~OL[~RS", user->recap, (user->vis) ? "" : "~RS~OG]~RS", inpstr);

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~FRthinks to ~RS", "~OL[~RS", user->recap, "~RS~OG]");
        write_room_level_except(user->level + 1, 1, 0, text, user, user->room, u);
        sprintf(text, "%s~RS ~FRthinks to ~RS", user->recap);
        write_room_level_except(user->level, 0, 0, text, user, user->room, u);
    } else {
        sprintf(text, "%s~RS ~FRthinks to ~RS", user->recap);
        write_room_except_both(user->room, text, user, u);
    ...
    if (!u->vis) {
        sprintf(text, "%s%s%s~RS~FR . o O (~RS %s~RS~FR )\n", "~OL[~RS", u->recap, "~RS~OG]", inpstr);
        write_room_level_except(u->level + 1, 1, 0, text, user, user->room, u);
        sprintf(text, "%s~RS~FR . o o (~RS %s~RS~FR )\n", u->recap, inpstr);
        write_room_level_except(u->level, 0, 0, text, user, user->room, u);
    } else {
        sprintf(text, "%s~RS~FR . o o (~RS %s~RS~FR )\n", u->recap, inpstr);
        write_room_except_both(user->room, text, user, u);
...
void sthink(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot sthink.\n");
	return;
    }

    if (word_count < 2) {
	write_user(user, "sthink what?\n");
	return;
    }
    ...
    vwrite_user(user, "~OLYou sthink . o O (~RS %s~RS ~FR~OL)~RS\n",inpstr);

    name = user->recap;

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OLsthinks . o O (~RS %s~RS~OL )\n", "~OL[~RS", name, "~RS~OG]", inpstr);
        write_level(user->level + 1, 1, 0, text, user);
        name = invisname;
        sprintf(text, "%s~RS ~OLsthinks . o O (~RS %s~RS~OL )\n", name, inpstr);
        write_level(user->level, 0, 0, text, user);
    } else {
        sprintf(text, "%s~RS ~OLsthinks . o O (~RS %s~RS~OL)\n", name, inpstr);
        write_room_except(NULL, text, user);
...
void sthink_to(UR_OBJECT user, char *inpstr) {
    char *name1, *name2;
    UR_OBJECT u;
    ...
    if (word_count < 3) {
        write_user(user, "usthink what to who?\n");
        return;
    }
    ...
    if (u->type == TYPE_LIMBO) {
        vwrite_user(user, notloggedon);
        return;
    }

    if (u == user) {
        write_user(user, "Why must you shout think to YOURSELF?\n");
        return;
    }
    ...
    vwrite_user(user, "~OLYou sthink to ~RS%s%s%s~RS~OL . o O (~RS %s~RS~OL )\n",
                (u->vis) ? "" : "~OL[~RS", name2, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
    vwrite_user(u, "%s%s%s~RS ~OLsthinks to you . o O ( ~RS%s~RS~OL )\n",
                (user->vis) ? "" : "~OL[~RS", name1, (user->vis) ? "" : "~RS~OG]", inpstr);
  
    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OLsthinks to ~RS", "~OL[~RS", name1, "~RS~OG]");
        write_level_except(user->level + 1, 1, 0, text, user, u);
        name1 = invisname;
        sprintf(text, "%s~RS ~OLsthinks to ~RS", name1);
        write_level_except(user->level, 0, 0, text, user, u);
    } else {
        sprintf(text, "%s~RS ~OLsthinks to ~RS", name1);
        write_room_except_both(NULL, text, user, u);
    ...
    if (!u->vis) {
        sprintf(text, "%s%s%s~RS~OL . o O (~RS %s~RS~OL )\n", "~OL[~RS", name2, "~RS~OG]", inpstr);
        write_level_except(u->level + 1, 1, 0, text, user, u);
        name2 = invisname;
        sprintf(text, "%s~RS~OL . o O (~RS %s~RS~OL )\n", name2, inpstr);
        write_level_except(u->level, 0, 0, text, user, u);
    } else {
        sprintf(text, "%s~RS~OL . o O (~RS %s~RS~OL )\n", name2, inpstr);
        write_room_except_both(NULL, text, user, u);
...
void sing_it(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
        write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot sing.\n");
        return;
    }
    ...
    vwrite_user(user, "~OL~FYyou sing o/~ ~FW%s ~FYo/~\n", inpstr);

    name = user->recap;

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OL~FYsings o/~ ~FW%s ~FYo/~\n", "~OL[~RS", name, "~RS~OG]", 
inpstr);
        write_room_level(user->level + 1, 1, 0, text, user, user->room);
        name = invisname;
        sprintf(text, "%s~RS ~FYsings o/~ ~RS%s~RS ~FYo/~\n", name, inpstr);
        write_room_level(user->level, 0, 0, text, user, user->room);
    } else {
        sprintf(text, "%s~RS ~OL~FYsings o/~ ~FW%s ~FYo/~\n", name, inpstr);
        write_room_except(user->room, text, user);
    }
...
void sing_to(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    char *name1, *name2;
    ...
    if (word_count < 3) {
        write_user(user, "Sing what to who?\n");
        return;
    }
    ...
    if (user->room != u->room) {
        write_user(user, "You must to be in the same room as them to do that, try .ussing.\n");
        return;
    }
    ...
    vwrite_user(user, "~FYYou sing to ~RS%s%s%s~FY o/~~RS %s~RS~FY o/~\n",
                (u->vis) ? "" : "~OL[~RS", u->recap, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
    vwrite_user(u, "%s%s%s ~FYsings to you o/~~RS %s~RS~FY o/~\n",
                (user->vis) ? "" : "~OL[~RS", user->recap, (user->vis) ? "" : "~RS~OG]~RS", inpstr);

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~FYsings to ~RS", "~OL[~RS", user->recap, "~RS~OG]");
        write_room_level_except(user->level + 1, 1, 0, text, user, user->room, u);
        sprintf(text, "%s~RS ~FYsings to ~RS", user->recap);
        write_room_level_except(user->level, 0, 0, text, user, user->room, u);
    } else {
        sprintf(text, "%s~RS ~FYsings to ~RS", user->recap);
        write_room_except_both(user->room, text, user, u);
    ...
    if (!u->vis) {
        sprintf(text, "%s%s%s~RS~FY o/~~RS %s~RS~FY o/~\n", "~OL[~RS", u->recap, "~RS~OG]", inpstr);
        write_room_level_except(u->level + 1, 1, 0, text, user, user->room, u);
        sprintf(text, "%s~RS~FY o/~~RS %s~RS~FY o/~\n", u->recap, inpstr);
        write_room_level_except(u->level, 0, 0, text, user, user->room, u);
    } else {
        sprintf(text, "%s~RS~FY o/~~RS %s~RS~FY o/~\n", u->recap, inpstr);
        write_room_except_both(user->room, text, user, u);
...
void ssing(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot ssing.\n");
	return;
    }

    if (word_count < 2) {
	write_user(user, "ssing what?\n");
	return;
    }
    ...
    vwrite_user(user, "~OLYou ssing o/~~RS %s~RS ~OLo/~\n",inpstr);

    name = user->recap;

    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OLssings o/~~RS %s~RS ~OLo/~\n", "~OL[~RS", name, "~RS~OG]", inpstr);
        write_level(user->level + 1, 1, 0, text, user);
        name = invisname;
        sprintf(text, "%s~RS ~OLssings o/~~RS %s~RS ~OLo/~\n", name, inpstr);
        write_level(user->level, 0, 0, text, user);
    } else {
        sprintf(text, "%s~RS ~OLssings o/~~RS %s~RS ~OLo/~\n", name, inpstr);
        write_room_except(NULL, text, user);
...
void ssing_to(UR_OBJECT user, char *inpstr) {
    char *name1, *name2;
    UR_OBJECT u;

    if (user->muzzled) {
        write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot ussing.\n");
	return;
    }

    if (word_count < 3) {
        write_user(user, "ussing what to who?\n");
        return;
    }
    ...
    if (u == user) {
        write_user(user, "Why must you shout sing to YOURSELF?\n");
        return;
    }
    ...
    vwrite_user(user, "~OLYou ssing to ~RS%s%s%s~RS ~OLo/~~RS %s~RS ~OLo/~\n",
                (u->vis) ? "" : "~OL[~RS", name2, (u->vis) ? "" : "~RS~OG]~RS", inpstr);
    vwrite_user(u, "%s%s%s~RS ~OLssings to you ~OLo/~~RS %s~RS ~OLo/~\n",
                (user->vis) ? "" : "~OL[~RS", name1, (user->vis) ? "" : "~RS~OG]", inpstr);
  
    if (!user->vis) {
        sprintf(text, "%s%s%s~RS ~OLssings to ~RS", "~OL[~RS", name1, "~RS~OG]");
        write_level_except(user->level, 1, 0, text, user, u);
        name1 = invisname;
        sprintf(text, "%s~RS ~OLssings to ~RS", name1);
        write_level_except(user->level - 1, 0, 0, text, user, u);
    } else {
        sprintf(text, "%s~RS ~OLssings to ~RS", name1);
        write_room_except_both(NULL, text, user, u);
    }
    ...
    if (!u->vis) {
        sprintf(text, "%s%s%s~RS ~OLo/~~RS %s~RS ~OLo/~\n", "~OL[~RS", name2, "~RS~OG]", inpstr);
        write_level_except(u->level, 1, 0, text, user, u);
        name2 = invisname;
        sprintf(text, "%s~RS ~OLo/~~RS %s~RS ~OLo/~\n", name2, inpstr);
        write_level_except(u->level - 1, 0, 0, text, user, u);
    } else {
        sprintf(text, "%s~RS ~OLo/~~RS %s~RS ~OLo/~\n", name2, inpstr);
        write_room_except_both(NULL, text, user, u);
    }
...
void emote(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot emote.\n");
	return;
    }
    ...
    if (user->type == CLONE_TYPE) {
	if (inpstr[0] == '\'' && (inpstr[1] == 's' || inpstr[1] == 'S'))
	    vwrite_room(user->room, "The clone of %s%s\n", user->recap, inpstr);
	else
	    vwrite_room(user->room, "The clone of %s~RS %s\n", user->recap, inpstr);
    ...
    vwrite_user(user, "%s%s%s~RS %s\n", (user->vis) ? "" : "~OL[~RS", user->recap, (user->vis) ? "" : "~RS~OG]", inpstr);

    if (!user->vis) {
	sprintf(text, "%s%s%s~RS %s\n", "~OL[~RS", name, "~RS~OG]", inpstr);
	write_room_level(user->level + 1, 1, 0, text, user, user->room);
	name = invisname;
	sprintf(text, "%s~RS %s\n", name, inpstr);
	write_room_level(user->level, 0, 0, text, user, user->room);
...
/* what the hell is this for?!?!?  finally, I figured it out, it is for an apostrophe!! Duh!!
if (inpstr[0]=='\'' && (inpstr[1]=='s' || inpstr[1]=='S')) sprintf(text,"%s~RS%s\n",name,inpstr);
else sprintf(text,"%s~RS %s\n",name,inpstr);
I'll add this back in sometime...*/ 
    record(user->room,text);
    bot_trigger(user,inpstr,0);
...
void semote(UR_OBJECT user, char *inpstr) {
    ...
    if (!user->vis) {
	sprintf(text, "~OL!!~RS %s%s%s~RS %s\n", "~OL[~RS", name, "~RS~OG]", inpstr);
	write_level(user->level + 1, 1, 0, text, NULL);
	name = invisname;
	sprintf(text, "~OL!!~RS %s~RS %s\n", name, inpstr);
	write_level(user->level, 0, 0, text, NULL);
    } else {
	sprintf(text, "~OL!!~RS %s~RS %s\n", name, inpstr);
	write_room(NULL, text);
    }
...
void pemote(UR_OBJECT user, char *inpstr) {
    ...
    if (u == user) {
	write_user(user, "You really shouldn't have to do that, you know.\n");
	return;
    }
    ...
    if (!u->vis)
        sprintf(text, "~OL<Pemote to ~RS%s%s%s~RS~OL>>>>~RS %s~RS %s\n", "~OL[~RS", name, "~RS~OG]", user->recap, inpstr);
    else
        sprintf(text, "~OL<Pemote to ~RS%s~RS~OL>>>>~RS %s~RS %s\n", name, user->recap, inpstr);
...
void echo(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot echo.\n");
	return;
    }
...
// Shout an echo
void shout_echo(UR_OBJECT user, char *inpstr) {

    if (user->muzzled) {
        write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot secho.\n");
        return;
    }
...
void mutter(UR_OBJECT user, char *inpstr) {
    ...
    if (user->room != u->room) {
	vwrite_user(user, "%s~RS is not in this room, so speak freely of them.\n", u->recap);
	return;
    }

    if (u == user) {
	write_user(user, "How in the universe do you expect to accompllish that?\n");
	return;
    }

    vwrite_user(user, "~FTYou say to everyone except ~RS%s~RS~FT:~RS %s\n", u->recap, inpstr);
    name = user->recap;

    if (!user->vis) {
	sprintf(text, "%s%s%s~RS ~FTsays to everyone except ~RS", "~OL[~RS", name, "~RS~OG]");
	write_room_level_except(user->level, 1, 0, text, user, user->room, u);
	name = invisname;
	sprintf(text, "%s~RS ~FTsays to everyone except ~RS", name);
	write_room_level_except(user->level - 1, 0, 0, text, user, user->room, u);
    } else {
	sprintf(text, "%s~RS ~FTsays to everyone except ~RS", name);
...
void wizshout(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot wizshout.\n");
	return;
    }
...
/*** Emote something to other wizes and BOTs. If the level isnt given it
     defaults to WIZ level. ***/
void wizemote(UR_OBJECT user, char *inpstr) {
    int lev;

    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot emote to the rest of the Wizzes.\n");
	return;
    }
...
int slen,lc,c,i,j;
char *clr[]={"~OL~FR","~OL~FG","~OL~FT","~OL~FW","~OL~FY"};

if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, you cannot greet.\n");  return;
  }
if (word_count<2) {
...
void bcast(UR_OBJECT user, char *inpstr, int beeps) {
    ...
    if (user->muzzled) {
	write_user(user, "~OLYou are ~FRmuzzled~FW, you cannot broadcast.\n");
	return;
    }
    ...
    write_monitor(user,NULL,0);
    vwrite_room(NULL, "\n\n%s~OL~FW--==[~FYBroad Cast Message~RS:~FW %s ~RS~OL~FW]==--\n\n", bp, inpstr);
}
...
void wake(UR_OBJECT user) {
    ...
    if (user->muzzled) {    
        write_user(user, "~OL~FRSystem: ~FWYou are muzzled, you cannot wake anyone.\n");
        return;
    }    
    ...
    if (u == user) {
	write_user(user, "~OL~FRSystem: ~FWWhat do you need to wake yourself for?\n");
	return;
    }

    if ((check_igusers(u, user)) != -1 && user->level < BOT) {
	vwrite_user(user, "%s~RS ~OL~FGis ignoring wakes from you.\n", u->recap);
	return;
    }

    if (u->afk) {
	write_user(user, "~OL~FGYou cannot wake someone who is ~FRAFK.\n");
	return;
    }
    ...
    if (!user->vis)
	vwrite_user(u, "\n%s~BR*** %s~FW%s%s~RS~BR says: ~OL~LIWAKE UP!!!!~RS~BR ***\n\n", bp, "~OL[~RS", name, "~RS~OG]");
    else
	vwrite_user(u, "\n%s~BR*** ~FW%s~RS~BR says: ~OL~LIWAKE UP!!!~RS~BR ***\n\n", bp, name);
}
...
void beep(UR_OBJECT user, char *inpstr) {
    ...
    if (user->muzzled) {
	write_user(user,"~OL~FRSystem: ~OLYou are muzzled, you cannot beep.\n");
	return;
    }
    ...
    if (u == user) {
	write_user(user,"~OL~FRSystem: ~FWNow thats just crazy... Go play some MP3's instead.\n");
	return;
    }
    ...
    if (u->ignall && (user->level < BOT || u->level > user->level)) {
	if (u->malloc_start != NULL)
	    vwrite_user(user, "%s~RS ~OL~FGis writing a message at the moment.\n", u->recap);
	else
	    vwrite_user(user, "%s~RS ~OL~FGis not listening at the moment.\n", u->recap);
	return;
    }
    ...
    if (!u->vis)
	vwrite_user(user, "\007~OL~FYYou beep ~RS%s%s%s~RS", "~OL[~RS", u->recap, "~RS~OG]");
    else
	vwrite_user(user, "\007~OL~FYYou beep ~RS%s~RS", u->recap);
    ...
    if (!user->vis)
	vwrite_user(u, "\07%s%s%s~RS ~OL~FYbeeps you", "~OL[~RS", user->recap, "~RS~OG]");
    else
	vwrite_user(u, "\07%s~RS ~OL~FYbeeps you", user->recap);

    if (word_count > 2)
	vwrite_user(u, ":~RS %s ", inpstr); 
    write_user(u, "~RS~FR -=[*] BEEP [*]=-\n");
}
...
void revedit(UR_OBJECT user) {
    ...
    if (!cnt)
	write_user(user, "EDIT review buffer is empty.\n");
    else
	write_user(user, "\n~BB~FG*** End ***\n\n");
}
...
void revafk(UR_OBJECT user) {
    ...
    if (!cnt)
	write_user(user, "AFK review buffer is empty.\n");
    else
	write_user(user, "\n~BB~FG*** End ***\n\n");
}
...
void revclr(UR_OBJECT user) {
    ...
    if (!user->vis) {
	sprintf(text, "%s%s%s~RS has cleared the review buffer.\n", "~OL[~RS", name, "~RS~OG]");
	write_room_level(user->level + 1, 1, 0, text, user, user->room);
	name = invisname;
	sprintf(text, "%s~RS has cleared the review buffer.\n", name);
	write_room_level(user->level, 0, 0, text, user, user->room);
    } else {
	sprintf(text, "%s~RS has cleared the review buffer.\n", name);
	write_room_except(user->room, text, user);
...
void revtell(UR_OBJECT user) {
    ...
    if (word_count < 2 || user->level < BOT)
        u = user;
    else {
        ...
        if (u->level >= user->level) {
            write_user(user, "You cannot see that persons tells, they are of equal or higher rank than you.\n");
            return;
        }
 
        vwrite_user(user, "~FT(Review of %s's tells)\n", u->name);
    }

    cnt = 0;
    for(i = 0; i < REVTELL_LINES; ++i) {
	line = (u->revline + i) % REVTELL_LINES;
	if (u->revbuff[line][0]) {
	    cnt++;
            if (cnt == 1)
        	write_user(user, "\n~BB~FG*** Tell Buffer ***\n\n");
            write_user(user, u->revbuff[line]); 
        }
    }
...
void status(UR_OBJECT user) {
    ...
    write_user(user, "\n");
    if (!on) {
	sprintf(text, "+--- ~FTUser Info~RS --- %s~RS %s~RS --- ~FY(NOT online)~RS ---+", u->recap, u->desc);
	write_user(user, center_string(78, 0, NULL, text));
    } else {
	sprintf(text, "+--- ~FTUser Info~RS --- %s~RS %s~RS --- ~OL(online)~RS ---+", u->recap, u->desc);
	write_user(user, center_string(78, 0, NULL, text));
    }
    sprintf(text, "Level: %s%s~RS\n", user_level[u->level].color, user_level[u->level].name);
    write_user(user, text);
    mins = (int) (time(0) - u->last_login) / 60;
    ...
    vwrite_user(user,"Gender: %s~RS          Age: %-8s %s\n", sex[u->gender], text2, text3);
    if (!strcmp(u->homepage, "#UNSET"))
	sprintf(homepg, "Currently unset");
    else
	strcpy(homepg, u->homepage);

    if (!strcmp(u->email, "#UNSET"))
	sprintf(email, "Currently unset");
    else {
	if (!u->hideemail)
    ...
    vwrite_user(user, "Exit Msg     : %s~RS %s~RS to ...\n", u->recap, u->out_phrase);
   ...
   if (u == user || user->level >= SYSOP) {
	...
	vwrite_user(user, "Unarrest Lev : %-14.14s  Arr lev: %-13.13s  Muz Lev : %-13.13s\n",
...
void examine(UR_OBJECT user) {
    ...
    vwrite_user(user, "Name: %-*.*s~RS Level: %s\n",
		44 + cnt * 3, 44 + cnt * 3, text2, user_level[u->level].color, user_level[u->level].name);
    if (!on) {
...
sprintf(text2,"%s~RS %s",u->recap,u->desc);
cnt=colour_com_count(text2);
sprintf(text,"~FY~OL###~RS %s~RS ~FY~OL###~RS\n\n",text2);
write_user(user,text);
sprintf(filename,"%s/%s/%s.P",USERFILES,USERPROFILES,u->name);
if (!(fp=fopen(filename,"r"))) write_user(user,"Insert your profile here (~FW~OL.entpro~RS)\n\n");
else {
        fclose(fp);
        more(user,user->socket,filename);
        }
sprintf(text,"Level      : %s%s\n", user_level[u->level].color, user_level[u->level].name);
write_user(user,text);
sprintf(text,"Age        : %-3s\n", text3);
write_user(user, text);
sprintf(text,"Sex        : %s\n", sex[u->gender]);
...
if (!on || u->type==TYPE_LIMBO) {
  ...
  vwrite_user(user,"Which was  : %d day%-1s, %d hour%-1s, %d minute%-1s ago\n",
	days2,PLTEXT_S(days2),hours2,PLTEXT_S(hours2),mins2,PLTEXT_S(mins2));
...
idle=(int)(time(0) - u->last_input)/60;
vwrite_user(user,"On for     : %d hour%-1s, %d minute%-1s\n",hours2,PLTEXT_S(hours2),mins2,PLTEXT_S(mins2));
if (u->afk) {
vwrite_user(user,"Idle for   : %d minute%-1s ~FR~OLAFK~RS, ~FYmessage: %-25s\n",idle,PLTEXT_S(idle),u->afk_mesg);
  }
else
vwrite_user(user,"Idle for   : %d minute%-1s\n",idle,PLTEXT_S(idle));
vwrite_user(user,"Total login: %d day%-1s, %d hour%-1s, %d minute%-1s\n\n",
		days,PLTEXT_S(days),hours,PLTEXT_S(hours),mins,PLTEXT_S(mins));
if ((newmail=mail_sizes(u->name,1))) vwrite_user(user,"~FT~OL%s has unread mail ~RS(~FR~OL%d~RS)~RS.\n\n",u->recap,newmail);
}
...
void set_attributes(UR_OBJECT user, char *inpstr, int xtraops) {
    int i = 0, tmp = 0, setattrval = -1, recapword = 2;
...
ATT_JUMP:
	...
	case SETGEND:
	    ...
	    if (xtraops == 2) {
		write_user(user, "Usage: gender m/f/n\n");
	    } else {
	...
	case SETAGE:
	    ...
	    tmp = atoi(word[2]);
	    if (tmp < 0 || tmp > 100) {
		write_user(user, "You can only set your age range between 0 (unset) and 100.\n");
		return;
	    }
	    user->age = tmp;
	...
	case SETMAIL:
	    switch(user->mail_notify) {
		case 0:
		    user->mail_notify = 1;
		    write_user(user, "Mail notify ON\n");
		    break;

		case 1:
                    user->mail_notify = 0;
                    write_user(user, "Mail notify OFF\n");
                    break;
	    }
	    return;
	...
	case SETEMAIL:
	    ...
	    if (!strcmp(user->email, "#UNSET"))
		write_user(user, "Email set to : ~FRunset\n");
	    else
		vwrite_user(user, "Email set to : ~FT%s\n", user->email);
	...
	case SETHOMEP:
		...
		vwrite_user(user, "Homepage set to : ~FT%s\n", user->homepage);
	    return;
	...
	case SETRECAP:
		...
		return;
	    } else if (strlen(colour_com_strip(inpstr)) > USER_NAME_LEN) {
		write_user(user, "Sorry, That recap is too long.\n");
		return;
	    }
	    if (word_count < recapword + 1) {
		write_user(user, "Usage: set recap <name as you would like it>\n");
		return;
	    }
	    if (strstr(word[recapword], "~LI") != NULL) {
		write_user(user, "You may not use the Blink tag in your recapped name.\n");
		return;
	    } else if (strstr(word[recapword], "~BK") != NULL) {
		write_user(user, "You may not use the Background Black tag in your recapped name.\n");
		return;
	    } else if (strstr(word[recapword], "~FK") != NULL) {
	        write_user(user, "You may not use the Front Black tag in your recapped name.\n");
	        return;
	    }

	    recname = colour_com_strip(word[recapword]);
	    strcpy(name, recname);
	    strtolower(name);
	    name[0] = toupper(name[0]);
	    if (strcmp(user->name, name) && user->level <= BOT) {
		write_user(user, "The recapped name still has to match your proper name.\n");
		return;
	    }
	    strcpy(user->recap, word[recapword]);
...
while (setstr[i].type[0]!='*') {
  ...
  switch(i) {
    case SETGEND:
      vwrite_user(user," %-10.10s : %-20.20s ", setstr[i].type, sexname[user->gender]);
      break;
    ...
    case SETWRAP:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, onoff[user->wrap]);
      break;
    ...
    case SETEMAIL:
      if (!strcmp(user->email,"#UNSET"))
	   ...
	   sprintf(text, "~OL%s~RS", user->email);
      vwrite_user(user," %-10.10s : %-20.20s ", setstr[i].type, text);
      break;
    ...
    case SETHIDEEMAIL:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, shide[user->hideemail]);
      break;

    case SETCOLOUR:
      vwrite_user(user," %-10.10s : ~FT~OL%-20.20s~RS ", setstr[i].type, onoff[user->colour]);
      break;

    case SETPAGER:
      sprintf(text, "%d", user->pager);
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, text);
      break;

    case SETROOM:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, rm[user->lroom]);
      break;

    case SETFWD:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, onoff[user->autofwd]);
      break;

    case SETPASSWD:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, onoff[user->show_pass]);
      break;

    case SETRDESC:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, onoff[user->show_rdesc]);
      break;

    case SETCOMMAND:
      vwrite_user(user, " %-10.10s : ~OL%-20.20s~RS ", setstr[i].type, cmd[user->cmd_type]);
      break;
    ...
    case SETALERT:
      vwrite_user(user, " %-10.10s : ~OL%-61.61s~RS ",setstr[i].type, onoff[user->alert]);
      break;

     case SETMAIL:
       vwrite_user(user, " %-11.11s: %-20.20s ", "mail notify", onoff[user->mail_notify]);
       break;
...
{
int hr,min,ign, newmail;

    if ((newmail = mail_sizes(user->name, 1)) && user->mail_notify)
	write_user(user, "~OL** YOU HAVE NEW MAIL **\n");
...
vwrite_user(user,"~FT[~RS%s~RS~FT, %02d:%02d, %02d:%02d, %s%s%s]:\n", TALKERNAME, thour, tmin, hr, min, user->bw_recap, !user->vis ? "+" : "", ign > 0 ? "!" : "");
}
...
if (user->command_mode) {
  write_user(user,"~FY~OLYou are now in ~RS~OLSpeech Mode~FY~OL, type .mode to return to Command Mode.\n");
  write_user(user,"~FT~OLIn Speech Mode you must use (~RS~OL.~RS~FT~OL) before typing any command or social.\n");
  user->command_mode=0;
  return;
  }
  write_user(user,"~FY~OLYou are now in ~RS~OLCommand Mode~FY~OL, type .mode to return to Speech Mode.\n");
  write_user(user,"~FT~OLIn Command Mode you ~FR~OLDO NOT~RS~FT~OL need to use (~RS~OL.~RS~FT~OL) before typing any command or social.\n");
user->command_mode=1;
}
...
if(!strcmp(word[1],"taasmoko"))
    ...
    if (user->level > NEW) 
    {
	  write_user(user, "This command is for new users only, you already have an account.\n");
	return;
    }
...
void afk(UR_OBJECT user, char *inpstr) {
	...
	if (!strcmp(word[1], "lock")) {
	    ...
	    if (user->type==REMOTE_TYPE) {
		 ...
		 they have no password to use. */
	         write_user(user, "Sorry, remote users cannot use the lock option.\n");
	         return;
            }
#endif
	    inpstr = remove_first(inpstr);
    ...
    if (!user->vis) {
	sprintf(text, "%s%s%s~RS ~OLgoes ~FRAFK\n", "~OL[~RS", user->recap, "~RS~OG]");
	write_room_level_except(user->level + 1, 1, 0, text, user, user->room, user);
	sprintf(text, "%s~RS ~OLgoes~FR AFK\n", user->recap);
	write_room_level_except(user->level, 0, 0, text, user, user->room, user);
    } else {
	vwrite_room_except(user->room, user, "%s~RS ~OLgoes~FR AFK", user->recap);
    }

    if (user->afk_mesg[0])
        vwrite_room_except(user->room, user, ": %s\n", user->afk_mesg);
    else
...
void visibility(UR_OBJECT user, int vis) {
	...
	write_user(user, "~OL~FWYou mumble an incantation under your breath and reappear.\n");
	vwrite_room_except(user->room, user, "~OL~FKYou hear a mumbled incantation and %s~RS~OL~FK materializes!\n", user->recap);
	user->vis = 1;
	return;
    }
    if (!user->vis) {
	write_user(user, "~OL~FRSystem: ~FWYou are already invisible.\n");
	return;
    }
    write_user(user, "~OL~FWYou mumble an incantation and fade out.\n");
    vwrite_room_except(user->room, user, "%s~RS ~OL~FKmumbles an incantation and disappears!\n", 
user->recap);
    user->vis = 0;
}
...
void make_invis(UR_OBJECT user) {
    ...
    if (user == u) {
	write_user(user, "~OL~FRSystem: ~FWThere is an easier way to make yourself invisible.\n");
	return;
    }
    if (!u->vis) {
	vwrite_user(user, "%s%s%s~RS ~OL~FKis already invisible.\n", "~OL[~RS", u->recap, "~RS~OG]~RS");
	return;
    }
    if (u->level >= user->level) {
	write_user(user, "~OL~FRSystem: ~FWYou cannot force invisibilty upon a user of equal or greater level.\n");
	return;
    }
    u->vis = 0;
    vwrite_user(user, "~OL~FWYou enact your will upon %s~RS ~OL~FWand make them become ~FKinvisible.\n", u->recap);
    vwrite_user(u, "%s~RS ~OL~FWhas willed you to become ~FKinvisible.\n", (user->vis) ? user->recap : invisname);
    vwrite_room_except(u->room, u, "~OL~FWYou watch %s~RS ~OL~FKmysteriously disappear!\n", u->recap);
}
...
void make_vis(UR_OBJECT user) {
    ...
    if (user == u) {
	write_user(user, "~OL~FRSystem: ~FWThere is an easier way to make yourself visible.\n");
        return;
    }
    if (u->vis) {
	vwrite_user(user, "%s~RS ~OL~FTis already visible.\n", u->recap);
	return;
    }
    if (u->level >= user->level) {
	write_user(user, "~OL~FRSystem: ~FWYou cannot force visibilty upon a user of equal or greater level.");
	return;
    }
    u->vis = 1;
    vwrite_user(user, "~OL~FTYou will %s~RS ~OL~FTto become visible.\n", u->recap);
    vwrite_user(u, "%s~RS ~OL~FThas willed you to become visible.\n", (user->vis) ? user->recap : invisname);
    vwrite_room_except(u->room, u, "~OL~FTYou gasp as you see %s~RS ~OL~FTappear before you!\n", 
u->recap);
}
...
switch(com_num) {
  ...
  case IGNBEEPS: 
      ...
      break;
  case IGNATMOS:
      switch(user->ignatmos) {
          case 0: user->ignatmos = 1;
                  write_user(user, "You will now ignore all atmospherics.\n");
                  break;
          case 1: user->ignatmos = 0;
                  write_user(user,"You will now hear all atmospherics.\n");
                  break;
          }
      break;
  default: break;
...
for(u=user_first;u!=NULL;u=u->next) {
  if (u->type==CLONE_TYPE && u->owner==user) {
    if (u->room==rm) {
      vwrite_user(user,"You already have a clone in %s.\n",rm->name);
      return;
      }	
...
if (rm==user->room) write_user(user,"~FY~OLYou wave your hands, mix some chemicals and a clone is created here.\n");
else vwrite_user(user,"~FY~OLYou wave your hands, mix some chemicals, and a clone is created in the %s.\n",rm->name);
if (user->vis) name=user->bw_recap; else name=invisname;
vwrite_room_except(user->room,user,"~FY~OL%s waves their hands...\n",name);
vwrite_room_except(rm,user,"~FY~OLA clone of %s appears in a swirling magical mist!\n",user->bw_recap);
}
...
for(u=user_first;u!=NULL;u=u->next) {
  if (u->type==CLONE_TYPE && u->room==rm && u->owner==u2) {
    destruct_user(u);
    reset_access(rm);
    write_user(user,"~FW~OLYou whisper a sharp spell and the clone is destroyed.\n");
    if (user->vis) name=user->bw_recap; else name=invisname;
    vwrite_room_except(user->room,user,"~FW~OL%s whispers a sharp spell...\n",name);
    vwrite_room(rm,"~FW~OLThe clone of %s shimmers and vanishes.\n",u2->bw_recap);
...
if (word_count<2) {
  ...
  write_user(user,"\n~FY~OLYou experience a strange sensation...\n");
  tu->room=user->room;
  user->room=rm;
...
if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, your clone cannot speak.\n");
  return;
  }
if (word_count<3) {
...
if (user->muzzled>1) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, your clone cannot emote.\n");
  return;
  }
if (word_count<3) {
...
void go(UR_OBJECT user) {
    ...
    if (nl!=NULL && !strncmp(nl->service, word[1], strlen(word[1]))) {
        ...
        /* If site is users home site then tell home system that we have removed
         him. */
        if (user->netlink == nl) {
            write_user(user, "~OLYou traverse cyberspace...\n");
            sprintf(text, "REMVD %s\n", user->name);
            write_sock(nl->socket, text);
            if (user->vis) {
		sprintf(text, "%s~RS goes to %s\n", user->recap, nl->service);
    ...
    if (rm == user->room) {
	vwrite_user(user, "You are already in %s!\n", rm->name);
	return;
    }
   /* See if link from current room */
	...
	//use .visit routines here!! mastamike and add trans rules
	vwrite_user(user, "To go to another user's home you must .visit them.\n");
	return;
    }
    move_user(user, rm, 1);
}
...
void move_user(UR_OBJECT user, RM_OBJECT rm, int teleport) {
	...
	if (teleport == 3) //give the user back their taxi fare.
	    user->money += 10;
	return;
    }
...
/* Reset invite room if in it */
    ...
    if (teleport == 1) {
	vwrite_room(rm, "%s~RS~OL appears in an explosion of blue magic!\n", user->recap);
	vwrite_room_except(old_room, user, "%s~RS~OL chants a spell and vanishes into a magical blue vortex!\n", user->recap);
	goto SKIP;
    }

    if (teleport == 2) {
	write_user(user, "\n~OLA giant hand grabs you and pulls you into a magical blue vortex!\n");
	vwrite_room(rm, "%s~RS~OL falls out of a magical blue vortex!\n", user->recap);
...
#endif
            vwrite_room_except(old_room, user, "~OLA giant hand grabs %s~RS~OL, who is pulled into a magical blue vortex!\n", user->recap);
        goto SKIP;
    }

    if (teleport == 3) {
	vwrite_room_except(old_room, user, "%s~RS~OL hails a cab, jumps in, and heads to %s.\n", user->recap, rm->name);
	vwrite_room(rm, "~OLA cab pulls up, and %s~OL hops out.\n", user->recap);
	goto SKIP;
    }

    vwrite_room(rm, "%s~RS %s.\n", user->recap, user->in_phrase);
    vwrite_room_except(user->room, user, "%s~RS %s to %s.\n", user->recap, user->out_phrase, rm->name);
...
void taxi_user(UR_OBJECT user, char *inpstr) {
    RM_OBJECT rm;

    if (word_count < 2) {
	write_user(user, "Usage: taxi <room>\n       A taxi costs 10 credits.\n");
	return;
    }
    ...
    if (rm == user->room) {
	vwrite_user(user, "You are already in that room!\n");
	return;
    }

    if (user->money < 10) {
	vwrite_user(user, "You only have %s credits on you. Wait a couple minutes or get some money\nfrom your bank.\n", user->money);
	return;
    }

    vwrite_user(user, "~OLYou hail a cab and head to %s.\n", rm->name);
    user->money -= 10;
    move_user(user, rm, 3);
}
...
vwrite_user(u,"%s~RS has invited you into %s.\n",name,rm->name);
u->invite_room=rm;
...
vwrite_user(user,"You knock asking to be let into %s.\n",rm->name);
vwrite_room_except(user->room,user,"%s~RS knocks asking to be let into %s.\n",user->recap,rm->name);
...
void clear_topic(UR_OBJECT user) {
	...
	if (rm->tlock <= user->level) {
	    rm->topic[0] = '\0';
	    write_user(user,"You have cleared the topic.\n");
	    name = user->recap;
	    if (!user->vis) {
		sprintf(text, "%s%s%s~RS has cleared the topic.\n", "~OL[~RS", name, "~RS~OG]");
		write_room_level(user->level + 1, 1, 0, text, user, rm);
		name = invisname;
		sprintf(text, "%s~RS has cleared the topic.\n", name);
		write_room_level(user->level, 0, 0, text, user, rm);
	    } else {
		sprintf(text, "%s~RS has cleared the topic.\n", name);
		write_room_except(rm, text, user);
	    }
	    ...
	    vwrite_user(user, "The topic is locked at level %s%s~RS.\n", 
		user_level[rm->tlock].color, user_level[rm->tlock].name);
	}
	return;
    }
	...
	if (user->level > command_table[CTOPIC].level || user->level >= SYSOP) {
		...
		if (rm->tlock <= user->level) {
		    rm->topic[0] = '\0';
	            write_room_except(rm, "\n~OLThe topic has been cleared.\n", user);
		}
            }
	    write_user(user, "All room topics that are  locked below your level have now been cleared\n");
	    return;
        }
...
u->lroom=2;
vwrite_user(u,"\n~FR~OLYou have been shackled to %s.\n",u->room->name);
vwrite_user(user,"~FR~OLYou shackled %s to %s.\n",u->bw_recap,u->room->name);
sprintf(text,"~FRShackled~RS to ~FY%s~RS by ~FY~OL%s~RS.\n",u->room->name,user->name);
add_history(u->name,1,text);
write_syslog(SYSLOG,1,"%s SHACKLED %s to: %s\n",user->name,u->name,u->room->name);
}
...
write_user(u,"You can now use the ~FTset~RS command to alter the ~FYroom~RS attribute.\n");
vwrite_user(user,"~FG~OLYou unshackled %s from the %s room.\n",u->bw_recap,u->room->name);
sprintf(text,"~FGUnshackled~RS from the ~FY%s~RS room by ~FY~OL%s~RS.\n",u->room->name,user->name);
add_history(u->name,1,text);
...
void set_topic(UR_OBJECT user, char *inpstr) {
    ...
    if (rm->tlock && rm->tlock > user->level) {
	vwrite_user(user, "The topic is locked at level %s%s~RS.\n", user_level[rm->tlock].color, user_level[rm->tlock].name);
	return;
    }
    ...
    if (!user->vis) {
	sprintf(text, "%s%s%s~RS has cchanged the topic to: %s\n",
		"~OL[~RS", name, "~RS~OG]", inpstr);
	write_room_level(user->level + 1, 1, 0, text, user, rm);
	name=invisname;
	sprintf(text, "%s~RS has changed the topic to: %s\n", name, inpstr);
	write_room_level(user->level, 0, 0, text, user, rm);
    } else {
	sprintf(text, "%s~RS has changed the topic to: %s\n", name, inpstr);
	write_room_except(rm, text, user);
...
switch (cnt) {
  ...
  case 3 : user->accreq=-1;
	   ...
	   strcpy(user->date,(long_date(1)));
	   ++amsys->level_count[user->level];
	   vwrite_user(user,"\n\07~OL*** You have been auto-promoted to level ~RS%s%s~RS~OL ***\n\n",user_level[user->level].color, user_level[user->level].name);
	   ...
	   sprintf(text,"~OL[ NEW USER ]~RS %s is now level ~RS%s%s~RS.\n",user->name, user_level[user->level].color, user_level[user->level].name);
	   write_level(NEW, 1, 0, text, NULL);
	   return;
  }
}
...
int lev;
 ...
 if (word_count<2) {
   write_user(user,"Usage: tempromo <user> [<level>]\n");
   return;
   }
 if (!(u=get_user_name(user,word[1]))) {
   write_user(user,notloggedon);
 ...
 if (u==user) {
   write_user(user,"~OL~FRSystem: ~FWYou can't promote yourself, temporarily or otherwise.\n");
   return;
   }
 /* determine what level to promote them to */
 if (u->level>=user->level) {
   write_user(user,"~OL~FRSystem: ~FWYou can't temporarily promote anyone of the same or greater level than you.\n");
   return;
   }
 if (word_count==3) {
   ...
   if (lev<=u->level) {
     vwrite_user(user,"~OL~FRSystem: ~FWYou must specify a level higher than ~FY%s ~FWcurrently is.\n",u->name);
     return;
     }
   }
 else lev=u->level+1;
 if (lev >= user->level) {
   write_user(user,"~OL~FRSystem: ~FWYou can't temporarily promote anoyone to a higher level than your own.\n");
   return;
   }
 ...
 u->level=lev;
 vwrite_user(user,"~OL~FGYou temporarily promote ~FT%s ~FWto ~FW%s.\n",u->name, user_level[u->level].name);
 vwrite_room_except(u->room, u, "%s~RS ~OL~FGstarts to glow as their power increases...\n",u->recap);
 vwrite_user(u,"~OL~FGYou have been promoted ~FT(temporarily) ~FWto level ~FW%s%s~RS~OL.\n",user_level[u->level].color, user_level[u->level].name);
...
// replace all this with do_promo? kelotz
  ...
  sprintf(text,"~OLWas promoted  by %s to level ~FW%s.\n", user->name, user_level[u->level].name);
  add_history(u->name,1,text);
  u->accreq=1;
  u->real_level=u->level;
...
vwrite_user(user,"You promote %s to level: %s.\n",u->name, user_level[u->level].name);
sprintf(text2,"~OLYou have been promoted to level: ~RS%s%s~RS~OL.\n",user_level[u->level].color, user_level[u->level].name);
...
if ((u=get_user(word[1]))!=NULL) {
  ...
  write_level(NEW, 1, 2, text, u);
  vwrite_user(u,"%s~RS~OL~FR has demoted you to level: ~FW%s%s~RS~OL!\n",name,user_level[u->level].color, user_level[u->level].name);
  write_syslog(SYSLOG,1,"%s DEMOTED %s to level %s.\n",user->name,u->name, user_level[u->level].name);
  sprintf(text,"~OL~FRWas demoted by %s to level ~RS%s.\n",user->name, user_level[u->level].name);
  add_history(u->name,1,text);
  add_user_date_node(u->name,(long_date(1)));
  u->vis=1;
  u->real_level=u->level;
...
strcpy(u->date,(long_date(1)));
save_user_details(u,0);
vwrite_user(user,"~OL~FRYou demote %s to level: ~FW%s%s~RS.\n", u->recap, 
user_level[u->level].color, user_level[u->level].name);
sprintf(text2,"~OL~FRYou have been demoted to level: ~FW%s%s~RS~OL.\n",user_level[u->level].color, user_level[u->level].name);
send_mail(user,word[1],text2,0);
write_syslog(SYSLOG,1,"%s DEMOTED %s to level %s.\n",user->name,word[1], user_level[u->level].name);
sprintf(text,"~OL~FRWas demoted by %s to level ~FW%s.\n",user->name, user_level[u->level].name);
add_history(u->name,1,text);
add_user_date_node(u->name,(long_date(1)));
destruct_user(u);
destructed=0;
...
if ((u=get_user(word[1]))!=NULL) {
  ...
  if (u->muzzled>=user->level) {
    vwrite_user(user,"%s~RS ~OLis already ~FRmuzzled.\n",u->recap);
    return;
    }
  vwrite_user(user,"%s ~OLnow has a muzzle of level: ~RS%s%s~RS~OL.\n",u->recap,user_level[user->level].color, user_level[user->level].name);
  write_user(u,"~OLYou have been ~FRmuzzled!\n");
...
if (u->muzzled>=user->level) {
  vwrite_user(user,"%s ~OLis already ~FRmuzzled.\n",u->name);
  destruct_user(u);
  destructed=0;
  return;
  }
...
strcpy(u->site,u->last_site);
save_user_details(u,0);
vwrite_user(user,"%s~RS~OL is given a muzzle of level: ~RS%s%s~RS~OL.\n",u->recap,user_level[user->level].color, user_level[user->level].name);
send_mail(user,word[1],"~OLYou have been ~FRmuzzled!\n",0);
write_syslog(SYSLOG,1,"%s muzzled %s (level %d).\n",user->name,u->name,user->level);
sprintf(text,"Level %d (%s) ~FRmuzzle~RS put on by %s.\n",user->level, user_level[user->level].name, user->name);
add_history(u->name,1,text);
destruct_user(u);
...
if ((u=get_user(word[1]))!=NULL) {
  ...
  if (!u->muzzled) {
    vwrite_user(user,"%s~RS ~OLis not ~FRmuzzled.\n",u->recap);
    return;
    }
  if (u->muzzled>user->level) {
    vwrite_user(user,"%s's ~OL~FRmuzzle ~FWis set to level ~FY%s~FW, you do not have the power to remove it.\n",u->name,user_level[u->muzzled].name);
    return;
    }
...
void arrest(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    RM_OBJECT rm;

    if (word_count < 3) {
	write_user(user, "Usage: arrest <user> <history message>\n");
	return;
    }
    word[1][0] = toupper(word[1][0]);
    if ((u = get_user(word[1]))) {
	...
	vwrite_user(user, "You place %s%s%s~RS under arrest.\n",
		(user->vis) ? "" : "~OL[~RS", u->recap, (user->vis) ? "" : "~RS~OG]");
        vwrite_user(u, "You have been arrested by %s~RS.\n", (user->vis) ? user->recap : invisname);
        if (rm == NULL) {
	    vwrite_user(user, "Cannot find the jail, so %s%s%s~RS is arrested but still in the %s.\n",
			(user->vis) ? "" : "~OL[~RS", u->recap, (user->vis) ? "" : "~RS~OG]", u->room->name);
	} else
	    move_user(u, rm, 2);

	if (!u->vis) {
	    sprintf(text, "%s%s%s~RS has been arrested by ", "~OL[~RS", u->recap, "~RS~OG]");
	    write_level_except(u->level + 1, 1, 0, text, u, user);
	    sprintf(text, "%s~RS has been arrested by ", invisname);
	    write_level_except(u->level, 0, 0, text, u, user);
	} else {
	    sprintf(text, "%s~RS has been arrested by ", u->recap);
	    write_room_except_both(NULL, text, u, user);
	}

	record_shout(text);

	if (!user->vis) {
	    sprintf(text, "%s%s%s~RS.\n", "~OL[~RS", user->recap, "~RS~OG]");
            write_level_except(user->level + 1, 1, 0, text, u, user);
            sprintf(text, "%s~RS.\n", invisname);
            write_level_except(user->level, 0, 0, text, u, user);
        } else {
            sprintf(text, "%s~RS.\n", user->recap);
	    write_room_except_both(NULL, text, u, user);
	}
...
if ((u=get_user(word[1]))) {
  ...
  if (user->level<u->arrestby) {
    vwrite_user(user,"%s~RS can only be unarrested by a %s%s~RS or higher.\n",u->recap,user_level[u->arrestby].color, user_level[u->arrestby].name);
    return;
    }
  --amsys->level_count[u->level];
  u->level=u->unarrest;
  vwrite_user(user, "level is now: %d\n", u->level);
  u->arrestby=0;
  user_list_level(u->name,u->level);
  strcpy(u->date,(long_date(1)));
  ++amsys->level_count[u->level];
...
/*** Change users password. Only BOTs and above can change another users 
...
if (word_count<3) {
  if (user->level<BOT) write_user(user,"Usage: passwd <old password> <new password>\n");
  else write_user(user,"Usage: passwd <old password> <new password> [<user>]\n");
  return;
  }
if (strlen(word[2])<3) {
  write_user(user,"~OL~FRSystem: ~FWNew password too short.\n");  return;
  }
if (strlen(word[2])>PASS_LEN) {
  write_user(user,"~OL~FRSystem: ~FWNew password too long.\n");  return;
  }
/* Change own password */
if (word_count==3) {
  if (strcmp((char *)crypt(word[1],crypt_salt),user->pass)) {
    write_user(user,"~OL~FRSystem: ~FWOld password incorrect.\n");  return;
    }
  if (!strcmp(word[1],word[2])) {
    write_user(user,"~OL~FRSystem: ~FWOld and new passwords are the same.\n");  return;
    }
  strcpy(user->pass,(char *)crypt(word[2],crypt_salt));
  save_user_details(user,0);
  cls(user);
  write_user(user,"~OL~FRSystem: ~FWPassword Changed.\n");
  add_history(user->name,1,"Changed passwords.\n");
  return;
  }
/* Change someone elses */
if (user->level<ROOT2) {
  write_user(user,"~OL~FRSystem: ~FWYou are not a high enough level to use the user option.\n");  
  return;
  }
...
#ifdef NETLINKS
  if (u->type==REMOTE_TYPE) {
    write_user(user,"~OL~FRSystem: ~FWYou cannot change the password of a user logged on remotely.\n");
    return;
    }
#endif
  if (u->level>=user->level) {
...
void kill_user(UR_OBJECT user) {
    ...
    if (user == victim) {
	write_user(user, "C'mon now, just use .quit and be done with it.\n");
        return;
    }
    if (victim->level >= user->level) {
	write_user(user, "You cannot kill a user of equal or higher level than yourself.\n");
	vwrite_user(victim, "%s~RS tried to kill you! Kick their ass!\n", user->recap);
	return;
    }
    write_syslog(SYSLOG, 1, "%s KILLED %s.\n", user->name, victim->name);
    write_user(user, "~OLYou chant an evil incantation...\n");
    if (user->vis)
	name = user->bw_recap;
    else
	name = invisname;
    vwrite_room_except(user->room, user, "~FW~OL%s chants an evil incantation...\n",name);
...
void suicide(UR_OBJECT user) {
    ...
    write_user(user, "\n\07~OL~LI*** WARNING - This will delete your account! ***\n\nAre you sure about this (y/n)? ");
    user->misc_op = 6;
    no_prompt = 1;
}
...
if (!load_user_details(u)) {
  ...
  strcpy(u->desc,"~OLis a~FG NEWphyte ~FWneeding a desc.");
...
if (stage==1) {
  ...
  if (!user->samesite_all_store) {
    ...
    else vwrite_user(user,"\nChecked ~FW~OL%d~RS users, ~FW~OL%d~RS had the site as ~FG~OL%s~RS ~FG(%s)\n\n",cnt,same,u->name,u->site);
    return;
    }
...
if (stage==2) {
  ...
  if (!user->samesite_all_store) {
    ...
    else vwrite_user(user,"\nChecked ~FW~OL%d~RS users, ~FW~OL%d~RS had the site as ~FG~OL%s\n\n",cnt,same,user->samesite_check_store);
    return;
    }
  /* check all the users.. */
  ...
  if (!found) vwrite_user(user,"No users have the same site as %s.\n",user->samesite_check_store);
    ...
    if (!on) vwrite_user(user,"\nChecked ~FW~OL%d~RS users, ~FW~OL%d~RS had the site as ~FG~OL%s~RS\n\n",cnt,same,user->samesite_check_store);
    else vwrite_user(user,"\nChecked ~FW~OL%d~RS users, ~FW~OL%d~RS had the site as ~FG~OL%s~RS\n\n",cnt,same,user->samesite_check_store);
    }
  return;
  } /* end of stage 2 */
...
write_user(user,LINEB);
vwrite_user(user,"~OL~FTSystem details for %s~RS (Amnuts version %s)  by KelotZ\n",talker_name,AMNUTSVER);
write_user(user,LINEB);
...
void toggle_shoutban(UR_OBJECT user) {
    if (amsys->shout_ban) {
        write_user(user, "~FRShouting ban has been turned: ~FTOFF.\n");
        sprintf(text, "%s turned the shout ban: OFF.\n", user->name);
        write_syslog(SYSLOG, 1, text);
        amsys->shout_ban = 0;
        return;
    }
    amsys->shout_ban = 1;
    write_user(user, "~FTShouting ban has been turned: ~FRON.\n");
    sprintf(text,"%s turned the shout ban: ON.\n", user->name);
    write_syslog(SYSLOG, 1, text);
}
...
if (user->room==NULL) {
  ...
  return;
	...
	vwrite_user(user,"^~%s: ~%sAmnuts %s VIDEO TEST~RS", colour_codes[col].txt_code, colour_codes[col].txt_code, AMNUTSVER);
	if (col % 2 == 1)
	    write_user(user, "    ");
	else
	    write_user(user, "\n");
...
void show_ranks(UR_OBJECT user) {
    ...
    null[0] = '\0';
    write_user(user, LINEA);
    vwrite_user(user, "| ~OL~FRLevels you can ascend to on ~RS%-60.60s~RS |\n", TALKERNAME);
    write_user(user, LINEA);
    for (i = NEW; i <= BOT; i++) {
	if (i == user->level)
	    ptr = cl;
	...
	if (i > USER && i < SYSOP) {
	    ...
	    vwrite_user(user, "%-38.38s~RS |\n", user_level[i].info);
	total += cnt[i];
    }
    write_user(user, LINEA);
    vwrite_user(user, "| There are %3d commands total on %-46.46s~RS           |\n", total, TALKERNAME);
    write_user(user, LINEA);
}
...
void wiz_list(UR_OBJECT user) {
    UR_OBJECT u;
    int some_on = 0, count = 0, visible = 0, cnt, i, inlist, mins, idle;
    char text2[ARR_SIZE], temp[ARR_SIZE], idlestr[20];
    struct wiz_list_struct *entry;
...
//		u = get_user(entry->name);
//	        sprintf(temp, "%-*s~RS  ", USER_NAME_LEN, u->recap);
		sprintf(temp, "%-*s  ", USER_NAME_LEN, entry->name);
	        strcat(text2, temp);
	        cnt++;
 	        inlist = 1;
	    }
    ...
    write_user(user,"~OL~FGThose Currently Online:\n");
    write_user(user,LINEY);
    for (u = user_first; u != NULL; u = u->next) {
	if (u->room != NULL)  {
	    if (u->level >= ROOT2) {
		if (!u->vis && (user->level < u->level && !(user->level >= ROOT2))) {
		     ++count;
		     continue;
		} else {
		    visible++;
		    ...
		    vwrite_user(user, " %-*.*s~RS %s%+15s~RS :%s~RS: ~OL%-11.11s~RS: %s\n",
	                        37+cnt*3, 37+cnt*3, text2, user_level[u->level].color, user_level[u->level].name,
				sex[u->gender], u->room->name, idlestr);
//		     vwrite_user(user, "%-*.*s : %15s : %s\n",43+cnt*3,43+cnt*3,text2,u->room->name,user_level[u->level].name);
	        }
	    }
	    some_on = 1;
	}
...
/*    if (count > 0)
	vwrite_user(user,"Wiz'z invisible to you: %d\n", count);
*/
    if (!some_on || (count > 0 && !visible))
	write_user(user, "Sorry, no wizzes visible at the moment...\n");
...
write_user(user,LINER);
vwrite_user(user,"  ~FT~OLAmnuts version %5s                          Jhong and Kelotz June 2002~RS \n",AMNUTSVER);
write_user(user,LINER);
...
if (user->room!=rm) {
  vwrite_user(user,"Don't be shooting in a public place.  Go to ~OL%s~RS to play.\n",rm->name);
  return;
  }
if (word_count<2) {
  if (user->bullets==0) {
...
if (!strcmp(word[2],"NORM")) {
  ...
  while (cmd!=NULL) {
	...
	write_user(user,"~OL~FWThat command is already at its normal level.\n");
	return;
        }
...
if (new_lev>user->level) {
  write_user(user,"~OLYou cannot set a command's level to one greater than your own.\n");
  return;
  }
found=0;
cmd=first_command;
while (cmd!=NULL) {
  if (!strncmp(word[1],cmd->name,strlen(word[1]))) {
    if (command_table[cmd->id].level>user->level) {
      write_user(user,"~OLYou are not a high enough level to alter that command's level.\n");
      return;
      }
    cmd->min_lev=new_lev;
    found=1;
...
if (u==user) {
  write_user(user,"~OL~FRSystem: ~FWYou cannot give yourself any commands.\n");
  return;
  }
...
write_user(user,"~OL+----------------------------------------------------------------------------+\n");
  ...
  if (!x) write_user(user,"~OL~FWUser has no given commands.\n");  
write_user(user,"~OL|----------------------------------------------------------------------------|\n\n");
  return;
  }
if (u->level>=user->level) {
  write_user(user,"~OL~FRSystem: ~FWYou cannot give commands to a user with the same or higher level as yourself.\n");
  return;
  }
cmd_id=-1;
for (cmd=first_command;cmd!=NULL;cmd=cmd->next) {
  if (!strncmp(word[2],cmd->name,strlen(word[2]))) {
    if (u->level>cmd->min_lev) {
      vwrite_user(user,"%s ~OL~FWcan already use that command.\n",u->name);
      return;
      }
    cmd_id=cmd->id;
    break;
...
if (cmd_id==-1) {
  write_user(user,"~OL~FRSystem: ~FWThat command does not exist.\n");
  return;
  }
/* check to see if the user has previously been banned from using the command */
if (has_xcom(u,cmd_id)) {
  write_user(user,"~OL~FRSystem: ~FWYou cannot give a command to a user that already has it banned.\n");
  return;
  }
/* user already has the command, so ungive it */
if (has_gcom(u,cmd_id)) {
  if (set_xgcom(user,u,cmd_id,0,0)) {
    vwrite_user(user,"~OL~FYYou have removed the given command '~FW%s~FY' for~FT %s\n",word[2],u->name);
    vwrite_user(u,"~OL~FYAccess to the given command '~FW%s~FY' has now been taken away from you.\n",word[2]);
    sprintf(text,"%s ~OL~FRUNGCOM'd~FY the command '~FW%s~FY'\n",user->name,word[2]);
    add_history(u->name,1,text);
...
if (set_xgcom(user,u,cmd_id,0,1)) {
  vwrite_user(user,"~OL~FWYou have given the '~FW%s~FW' command for ~FY%s\n",word[2],u->name);
  vwrite_user(u,"~OL~FWYou have been given access to the command '~FW%s~FW'.\n",word[2]);
  sprintf(text,"%s ~OL~FGGCOM'd~FW the command '~FW%s~FW'\n",user->name,word[2]);
  add_history(u->name,1,text);
...
if (banned) {
  ...
  if (!set) {
    ...
    write_user(user,"~OL~FRERROR:~FW Could not unban that command.\n");
    return 0;
    } /* end if */
  ...
  vwrite_user(user,"%s ~OL~FThas had the maximum amount of commands banned.\n",u->name);
  return 0;
  } /* end if */
...
vwrite_user(user,"%s ~OL~FThas had the maximum amount of commands given.\n",u->name);
return 0;
...
if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, you cannot speak to your friends.\n");
  return;
  }
...
if (user->muzzled) {
  write_user(user,"~OLYou are ~FRmuzzled~FW, you cannot emote to your friends.\n");  return;
  }
if (word_count<2) {
...
if (u->level>=user->level && user->level!=BOT) {
...
/* shift words down to pass to exec_com */
    for (w = 2; w < word_count; ++w) 
	strcpy(word[w - 2], word[w]);
    word[w][0] = '\0';
    word[w + 1][0] = '\0';
    word_count -= 2;

    vwrite_user(u, "%s~RS forces you to: '%s'\n", (user->vis) ? user->recap : invisname, inpstr);
    vwrite_user(user,"You force %s%s%s to: '%s'\n",
		(user->vis) ? "" : "~OL[~RS", u->recap, (user->vis) ? "" : "~RS~OG]~RS", inpstr);
    if (!exec_com(u, inpstr))
	vwrite_user(user, "Unable to execute the command for %s.\n", u->name);
    prompt(u);
}
...
strtolower(word[1]);
/* bounce everyone out - except BOTS */
if (!strcmp(word[1],"all")) {
  for (u=user_first;u!=NULL;u=u->next) {
    if (u==user || u->room!=rm || u->level==BOT) continue;
...
if (!strcmp(word[1],"-l")) {
  ...
  write_user(user,"|----------------------------------------------------------------------------|\n");
  if (rmcnt) {
    vwrite_user(user,"| Total personal rooms : ~OL~FW%2d~RS                                                  |\n",rmcnt);
    write_user(user,"|----------------------------------------------------------------------------|\n");
    }
  return;
  }
...
if (on) {
  ...
  write_room_except(u->room,"~OL~FWThere is a shimmering in the air and something pops into existence...\n",u);
  }
else {
  u->socket=-2;
  strcpy(u->site,u->last_site);
...
void watcher(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;
    int idlemin, idlesec;

    if (user->watching[0] != '\0' && !(u = get_user_name(user, user->watching))) {
	vwrite_user(user, "%s has logged off, you have stopped watching them.\n", user->watching);
	user->watching[0] = '\0';
    }


    for (u = user_first; u != NULL; u = u->next) {
	if (!strcmp(user->name, u->watching)) {
   	    idlemin = (int) (time(0) - user->last_input) / 60;
	    idlesec = (int) (time(0) - user->last_input) - (idlemin * 60);
	    if (u->level >= BOT && !user->login)
		vwrite_user(u, "~FG%d~FT/~FG%d ~FR%d~FW:~FR%02d~FW:~FR%02d~RS %s%s%s~RS ~OLIdle %d:%02d,~RS ~FRtyped~FY:~RS %s\n",
		tmonth + 1, tmday, thour, tmin, tsec, (user->vis) ? "" : "~OL[~RS", user->recap,
		(user->vis) ? "" : "~RS~OL]", idlemin, idlesec, inpstr);
	}
    }
}

void watchuser(UR_OBJECT user, char *inpstr) {
    UR_OBJECT u;

    if (user->real_level < BOT) {
	write_user(user, "What are you thinking?\n");
	return;
    }

    if (word_count < 2) {
	write_user(user, "You are currently watching ");
	if (strlen(user->watching) > 1)
	    vwrite_user(user, "%s~RS.\n", user->watching);
	else
	    write_user(user, "no one.\n");
	write_user(user, "Usage: watchuser [-n / <username>]\n    -n will end your watching.\n");
	return;
    }

    if (!strcmp(word[1], "-n")) {
	if (!user->watching)
	    write_user(user, "You are not watching anyone.\n");
	else {
	    vwrite_user(user, "You have stopped watching %s~RS.\n", user->watching);
	    user->watching[0] = '\0';
	}
        return;
    }
    ...
    if (u = get_user_name(user, word[1])) {
	if (!strcmp(u->name, user->watching)) {
	    vwrite_user(user, "You are already watching %s~RS\n", user->watching);
	    return;
	}

	if (u->level >= BOT) {
	    write_user(user, "No spying on each other...\n");
	    return;
	}

	vwrite_user(user, "You are now watching %s~RS.\n", u->recap);
